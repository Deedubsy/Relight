    ...
    public sealed class TurretPhase : ITickPhase
    {
        public void Tick(SimContext ctx, SimState st, double dt)
        {
            ...
            for (var i = 0; i < st.Machines.Count; i++)
            {
                ...
                if (target == 0) target = EnemyQueries.Nearest(ctx, st, cx, cy, def.RangeTiles);
        ...
        /// blinds the engineer's rifle and exactly as the placement preview promised (GP-W3).
        /// </summary>
        private static bool Eligible(SimContext ctx, SimState st, TurretDef def, double cx, double cy, int id)
        {
            ...
            var dx = p.X - cx;
            var dy = p.Y - cy;
            if (Math.Sqrt(dx * dx + dy * dy) > def.RangeTiles) return false;

...
System.IO.File.WriteAllText(log,"Normal starting inventory: "+sim.State.Engineer.Inv[Relight.Sim.ItemId.Steel]+" steel, "+sim.State.Engineer.Inv[Relight.Sim.ItemId.Copper]+" copper\n");
...
Check(sim.State.Engineer.Inv[Relight.Sim.ItemId.Steel]==20 && sim.State.Engineer.Inv[Relight.Sim.ItemId.Copper]==5,"Normal opening stock");
Check(shell.Active=="inventory-panel","Tab opens Backpack");
yield return Tap(UnityEngine.InputSystem.Key.Tab);Check(string.IsNullOrEmpty(shell.Active),"Tab closes Backpack");
yield return Tap(UnityEngine.InputSystem.Key.B);Check(shell.Active=="build-panel","B opens Build");
yield return Tap(UnityEngine.InputSystem.Key.Escape);Check(string.IsNullOrEmpty(shell.Active),"Escape closes Build");
UnityEngine.ScreenCapture.CaptureScreenshot("E:/Factorio2/Unity/Docs/evidence/ux-pass/after-hud-1080.png");
...
float until=UnityEngine.Time.realtimeSinceStartup+15;double goal=item==Relight.Sim.ItemId.Steel?30:10;bool captured=false;

...
var full=Add("excavator",64,352,Relight.Sim.Dir.N);full.Inv.Add(Relight.Sim.ItemId.Steel,1);
var offline=Add("excavator",51,355,Relight.Sim.Dir.N);
var empty=Add("generator",69,355);
var standby=Add("generator",75,355);standby.Inv.Add(Relight.Sim.ItemId.Coal,5);
var free=Add("belt",52,362,Relight.Sim.Dir.E);
st.Ledger=Relight.Sim.Ledger.Open(st,sim.Context.Data);
st.Admin.Lighting=1;st.Admin.Invulnerable=true;
System.IO.File.WriteAllText("E:/Factorio2/Unity/Docs/evidence/activity-animation/ids.txt",$"dig={dig.Id},full={full.Id},offline={offline.Id},empty={empty.Id},standby={standby.Id},free={free.Id},gen={gen.Id}");
return new {dig=dig.Id,full=full.Id,offline=offline.Id,empty=empty.Id,standby=standby.Id,free=free.Id,gen=gen.Id,pipeline=UnityEngine.Rendering.GraphicsSettings.currentRenderPipeline?.GetType().FullName};

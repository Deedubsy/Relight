// REL-58 look: interface scale 150% on the running game's PanelSettings (restored to 100 before Play stops).
...
foreach (var p in set) { sb.Append(p.name + " was " + p.scale + "; "); p.scale = 150 / 100f; }
return sb.ToString();

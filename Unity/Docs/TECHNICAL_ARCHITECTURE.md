...
- **Save schema version:** started clean at **v1**; **v2 since 2026-09-12** (a pending hand-craft refund is saved state, U-M-37); **since 2026-09-13 a v1 file is read** with those two fields defaulted to zero after its own checksum is verified as written, and the file is never rewritten (`SaveUpgrade`, U-M-38) — a default for a field this port's earlier build did not write, not a converter; every other version is refused by number. There is no version 0, no compatibility branch and no reader for reference `SaveFile` v1–3 (Q01/Q07, §9.3).
- **What a Unity save is bound to — decided for development (U-D-69 b, owner 2026-09-22; written down 2026-09-23, REL-63).** A save names the exact map it was made on. The real city's map id is the imported baseline plus `-scene-` and a fingerprint of the scene (`SceneWorld.Fingerprint`: size, spawn, every tile's kind, patch and solid flag, every site), so **any scene edit gives a new id**, and `SaveSerializer.MapProblem` refuses a save from any other id — two ids that share the baseline and differ only in the fingerprint are still different maps. The refusal is not "damaged": the file stays where it is, unchanged, and the Load screen shows it disabled with the reason. **While the game is unreleased, that is the rule: a map change or a save-version change may stop old Unity saves loading, and the player starts a new game.** No save has to be carried across; a `SaveUpgrade` step for a new schema version stays optional, and the existing upgrades are kept. Pinned by `SaveMapBindingTests.DuringDevelopmentASceneEditOrphansEverySave`. Loosening it (binding to a hand-bumped map version, or relocating and validating on load) is a new owner decision, and the rule **must be revisited before any release**. Reference saves are a separate matter (§9.3, U-D-27) and are unchanged.

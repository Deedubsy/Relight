...
| A save never changes the light | The lit mask and the relief memory are cleared only when a save is **read**. Writing a save and hashing the state walk the live state too; clearing there left every tile unlit for the tick after each autosave, because turrets and aliens read the mask before the light phase rebuilds it (found in L-02's review, fixed, two tests). On a load the mask is built before the loaded state is attached, for the same reason. Since REL-9 (2026-09-22) that build sits in `Simulation.Wrap`, which every loader goes through, and the mask is also brought up to date at the start of the Combat slot (`LightMaskPhase`), so turrets and aliens read this tick's light whatever happened between ticks. Presentation only reads the mask; it never builds it. |

...
#### 4.7.1 The validation rules to port (from `packages/sim/src/city/validateRiverfront.ts` and `packages/sim/src/firstRegion.ts`) — **Implemented 2026-09-22** as `Editor/World/CityValidator.cs` (REL-31); code and automated checks only

**Where the port is recorded:** `Assets/Relight/Editor/World/CityValidatorRules.md` lists every rule, with what was ported with a difference and what was not ported and why. It runs on the imported export today; running it on content authored in Unity after handover (D-08, E-15) is not built.

...
|---|---|---|
| **Q02** | The ending stays **genuinely unresolved**. No ending is to be invented, and commissioning the third plant must never set victory by itself. Blocks final campaign-completion work only. **Superseded 2026-09-20:** the owner decided the ending (U-D-60; GAME_DESIGN.md §11.1, approved, not implemented). Three plants still never complete the campaign. | §5.3 (M4), §11, §14 row 17 |
...
| 5 | **Automatic replenishment demonstrated through real delivery** | The step clears when a produced bullet batch reaches a turret along belts/inserters — or, since **U-D-53**, when a turret the line genuinely supplies is too full to accept one. Hand loading still never counts: the second case needs a real producer connected to that turret and no output until a batch has actually been made, and it exists because a player who keeps the turret topped up by hand leaves the line nothing to deliver and would otherwise never clear the step | **IR** | `supplyChainReaches`, `noteTurretSupply`; U-D-13, amended by U-D-53 |
...
Three strongholds are declared: `STRONGHOLDS = ['freight', 'quarry', 'wharf']` (`gameplayProgress.ts:1`). Each needs **3 Alien Keys** (`strongholdProgress` returns `required: 3`), drawn from up to **four eligible exterior camps** where the map geometry allows — the fourth is a documented placement limitation in the first region, not a design change (GP_CHECKPOINT). **In the port each stronghold has exactly three key camps**, one key each (U-D-69 (a), `FREIGHT_STRONGHOLD_DESIGN_2026-09-18.md` §11); the reference's fourth-camp allowance is not carried over. Key IDs are validated against `^<stronghold>:camp:[1-4]$` and must appear in `claimed`; an `opened` stronghold with fewer than 3 keys and no `inherited` flag is an invalid save.
...
**Port, as built (batch 4, 2026-09-23; U-D-69 (a), U-D-73; REL-136 to REL-146).** The Freight chapter follows `FREIGHT_STRONGHOLD_DESIGN_2026-09-18.md`. Code and automated checks only; nobody has played it.

- **Three key camps**: West passage (12 guards, U-D-64 (b)), Old utility (22) and Northwood approach (20). Each is found within 32 tiles, and its garrison is born whole, never within 12 tiles of the engineer. A key camp is never refilled.
- **Small loot camps**: four corridor camps (`freight:small:1..4`) of 5–8 skitters. Each is found within 12 tiles, carries no key, and is claimed on its last kill. It refills 900 s after clearing unless a player machine stands within 20 tiles, and keeps its emptied crate.
- **Occupation**: a key camp's key is claimed after 30 s with the engineer up, within 3 tiles of the marker, and no living guard of that camp within 20 tiles. Stepping out, going down or a guard's return resets the clock. The kite measurement (`evidence/freight-kite/`) found that walking to the marker while guards live never starts the clock.
- **Cache crate**: each key camp's garrison sets down a camp-bound chest beside the marker. It cannot be picked up and is not Home stock.
- **Doors, arena and guardian**: the warehouse doors stay shut until the engineer holds all three keys at a door. The arena holds 60 bodies and the 600 HP guardian. The guardian's charge is a straight line; at 300 HP it enters phase 2 and wakes the arena; it drops the Power core where it dies.
- **Carry**: the Power core takes both hands. Sprint, the dodge, firing and building are refused while it is carried, and going down drops it where the engineer fell.
- **Plant preparation**: Riverside Works is prepared once its squat is cleared, using the repair cost carried in the Backpack. It is commissioned with the core in both hands and the commission cost. It then supplies 600 kW, and the next major raid is aimed at it. A plant at 0 HP falls and stops supplying; no repair exists yet (open for the owner).
...
| M4 | Third plant, city-scale production and the final objective | Third plant and city-scale production are **in scope**; the **final objective was decided on 2026-09-20** (U-D-60, §11: switch the city on; approved, not implemented). Reaching M4 never sets victory |
...
## 11. Ending and campaign completion — Approved but not implemented

**Status: Approved but not implemented. Decided by the owner on 2026-09-20 (U-D-60), which resolves U-Q-02 / Q07 within the recorded direction D-GP-13.** Nothing here is built, and none of it has been seen running. [DECISIONS.md](DECISIONS.md) U-D-60 holds the owner's words; [ALWAYS_DARK_SPEC.md](ALWAYS_DARK_SPEC.md) owns what "lit" means.

### 11.1 The ending: switch the city on

- **The goal.** The campaign completes when every lighting district is live through the Master Switch and the raid declared by the last throw is repelled. A lighting district is the set of tiles whose nearest substation site is the same one (ALWAYS_DARK_SPEC §6); the full map has nine substation sites, so there are nine. This is not the Home, Riverside, Ironworks, Civic region order (D-04). The sun stays gone. Commissioning the third plant still never completes the campaign (U-D-23).
- **The Master Switch.** One named restoration project at a fixed city site, with a bill of materials drawn from all four factories. The site and the amounts are not set (F-05a proposes them).
- **Throwing it.** The player chooses the moment. The throw announces the final raid through the existing director, with the normal warning and one target. The switch then brings in each already-connected district in turn, one stage per district. Each stage puts a surge load on the circuit that feeds that district's substation, so power stays local: a district on a thin line browns out its own lamps, and turrets near it lose reach (U-D-59). A stage completes when that circuit has held the surge for the hold time at or above the throttle floor (placeholders: 60 s, 0.5; U-P-13), and the surge then lifts. The surge size is not set.
- **If it goes wrong.** If that circuit dies, or the Home core is disabled, the switch trips. Every finished stage stays live. The player repairs, adds generation and throws again for the rest. There is no defeat screen. A throw with only some districts connected is allowed; every throw costs a full final-tier raid, so lighting one district at a time is the harder road, not a shortcut.
- **What a live district is.** Fully lit in the sim's mask on every street tile, picture and rule together. The city's 18 streetlights are too few to be a visible result. As stages land, the raid loses its dark lanes and the turrets gain reach; the guaranteed raid route (ALWAYS_DARK_SPEC §5.2) still holds.
- **Afterwards.** A completion screen states the time played. The player may keep playing in the lit city, with raids ended.
- **The light board.** The city map (TASKS D-07) draws each lighting district as dark, connected, browned-out or live, marks Home, the factories, plants, strongholds, tram stops, the Master Switch, the engineer and the truck, always shows the announced raid target, shows its direction only once the Radio is earned, and states the districts live out of nine. It never moves the camera or pauses the game, and nothing is ordered from it. It does not depend on the ending and can be built before it.

What an implementer still must not do: set the surge size, the bill of materials or the site as if they were decided; add a score, a survival timer or an evacuation; or treat three plants as completion.

### 11.2 History: how this stayed open until 2026-09-20

Until U-D-60 this section read "Unresolved. Do not design this." The record is kept because other documents cite it.
...
| 7 | ~~Handcrafting requires a stationary player~~ — **superseded by U-D-44 (2026-09-15)**. The Home workshop processes its queue unattended at the same speeds; what remains required is the accessible cancel and the correct accounting | Reference: `flow.ts:1465-1466` mover/weapon/mining hand blocked; `flow.ts:1007` cancels on down or leaving Home; `panel.ts:712` closing the workshop cancels — **all four retired in the port**. Retained: Escape/Cancel returns every unprocessed ingredient, nothing is destroyed, and a second cancel returns nothing | **Verified as the reference's behaviour; deliberately not ported** (DECISIONS.md U-D-44) |

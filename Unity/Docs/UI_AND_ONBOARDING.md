...
**Port change (U-D-58, owner 2026-09-20):** the port is always dark and has no days, so nothing in the port states a Day. A row reads `Slot 1 · 1:12:30 played · saved 11 Sep 2026, 20:14`; the overwrite body is "{slot label} ({playtime} played, saved {time}) will be replaced."; the delete body is "{slot label} · {playtime} played · saved {time}. This cannot be undone."; the autosave notice is "Autosaved · 1:12:30 played." A save with no play accumulator still says `1:12:30 sim time`. The Day wording in this section and in §2.7.1 below describes the reference.
...
| Status strip | `hud-status-strip` | Clock + power: `Day N · HH:MM[ · Paused]` and `Name: D / S kW`. (Port, U-D-58: the clock is elapsed time `H:MM:SS[ · Paused]` with an `In light` / `In the dark` cue under it; `ALWAYS_DARK_SPEC.md` §3.) |
...
| **Alerts** | **Implemented and retained** | At most one highest-priority urgent strip plus three short transient notices; a keyed inbox preserves dismissed history and marks resolved conditions; one row updates with a repeat count rather than stacking. Base damage / engineer danger outranks schedule, then production. Dismissal never clears a real danger indicator (`RI-02B_UI_SPEC.md:144`). *Unity port (REL-86, 2026-09-22):* the port never had a way to dismiss a row. `HudNotices.Dismiss` had no caller and the rows are click-through, so REL-86 removed it. Live rows beyond the three shown are counted instead, as “+N more” under the third row, and the count drops as they clear. Any action on a row (dismiss, or UI-02b's proposed click-to-locate) is left to UI-02b. |

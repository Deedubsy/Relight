...
function upgrade(f: FlowState): FlowState {
  ...
  for (const k of STORE_ITEMS) if(k!=='concrete') f.store[k] ??= 0;
  for (const k of ITEMS) if(k!=='concrete') f.stats.delivered[k] ??= 0;

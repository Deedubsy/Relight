...
import {ITEMS,SHOT,GENERATOR_COAL_CAP,ASM_INPUT_MULT,ASM_OUTPUT_CAP,recipeOf,recipeOutput,isProcessor,isItem,machineById,accepts,KIND_LABEL,type Machine,type Item} from './flow';
...
export function machineInventory(m:Machine):Record<string,number> {
 if(m.kind==='turret')return {magazine:Math.floor((m.inv.rounds??0)/SHOT.count)};
...
export function machineInventoryDetail(m:Machine):string {
 if(m.kind==='turret')return `${m.inv.rounds??0} / ${hopperCapacity(m,TURRET_HOPPER)} rounds · 1 magazine = ${SHOT.count} rounds · ${Math.floor((m.inv.rounds??0)%SHOT.count)} loose rounds stay loaded`;
 ...
 if(isProcessor(m)){const r=recipeOf(m);return `Inputs: ${Object.entries(r.inputs).map(([k,n])=>`${m.inv[k]??0}/${n*ASM_INPUT_MULT} ${itemName(k)}`).join(' · ')}. Finished: ${m.out}/${ASM_OUTPUT_CAP} ${itemName(recipeOutput(r))}. In-progress ingredients are already committed.`;}
 return `Output: ${m.hold?'1 '+itemName(m.hold):'empty'} / 1 item`;
}
export interface MachineTransfer {type:'machineTransfer';id:number;item:string;n:number;put:boolean;expected?:{total:number;layout?:string;slot?:number}}
export function machineTransferPreview(st:SimState,id:number,item:string,n:number,put:boolean):{moved:number;reason:string} {
 ...
 if(put){const copy={...m,inv:{...m.inv}},have=Math.min(n,Math.floor(st.engineer.inv[item]??0));let moved=0;
  while(moved<have&&accepts(st,copy,item)){if(m.kind==='turret')copy.inv.rounds=(copy.inv.rounds??0)+SHOT.count;else copy.inv[item]=(copy.inv[item]??0)+1;moved++;}
...
export function machineTransfer(st:SimState,a:MachineTransfer):{ok:boolean;moved:number;reason:string} {
 ...
 if(a.expected){const total=a.put?(st.engineer.inv[a.item]??0):(machineInventory(m)[a.item]??0);
  if(total!==a.expected.total)return no('Source changed; select the stack again');
 ...
 const p=machineTransferPreview(st,a.id,a.item,a.n,a.put);if(!p.moved)return no(p.reason);
 const k=a.item as Item;
 if(a.put){if(m.kind==='turret'){m.inv.rounds=(m.inv.rounds??0)+p.moved*SHOT.count;st.flow!.stats.handFed+=p.moved;st.flow!.stats.handFedMags+=p.moved;}else{m.inv[k]=(m.inv[k]??0)+p.moved;if(m.kind==='generator'){st.flow!.stats.handFed+=p.moved;if(k==='coal')st.flow!.stats.handFedCoal+=p.moved;}}pocketDrop(st.engineer,k,p.moved);
  if(a.expected){const i=a.expected.slot!;slots[i]!.count-=p.moved;if(slots[i]!.count<=0)slots[i]=null;st.engineer.pack=slots;}
 }else{pocketTake(st.engineer,k,p.moved);let left=p.moved;
  if(m.kind==='turret'){m.inv.rounds-=left*SHOT.count;left=0;}

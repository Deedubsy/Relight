...
import {campaignWarning,knownCampaignThreat,openingDirection} from './campaignThreat';
...
export function campaignAlerts(st:SimState):CampaignAlert[] {
 ...
 for(const c of st.flow?.threat?.crawlers??[])if(c.hp>0&&Math.hypot(c.x-e.x,c.y-e.y)<=DANGER_R&&(c.kind!=='shade'||litAt(st,Math.floor(c.x),Math.floor(c.y))))out.push({id:`nearby:${c.id}`,priority:85,title:`Nearby threat · ${c.gp?.kind??c.kind} #${c.id}`,detail:'Hostile within close range of the engineer.',location:{x:c.x,y:c.y}});
 ...
 if(target){const intro=d.opening?.status==='active'&&d.minor?.id===d.opening.id,active=target.phase==='assault'||target.phase==='minor raid',title=target.phase==='approaching'?`Small enemy group approaching · from the ${openingDirection(st)}`:intro&&active?'Small enemy group attacking':active?'Base under attack':target.phase==='recovery'?'Base core disabled':target.phase==='withdrawal'?'Attackers withdrawing':`Assault in ${Math.max(0,Math.ceil(((d.major?.startsAt??st.t)-st.t)/60))} min`;out.push({id:`threat:${target.block}`,priority:active||target.phase==='approaching'?90:target.phase==='recovery'?80:60,title:`${title} · ${blockName(st,target.block)}`,detail:campaignWarning(st),location:{x:target.x,y:target.y}});}
 ...
 out.push({id:'schedule',priority:50,title:d.clock?`Major assault · ${Math.max(0,Math.ceil((d.clock.nextStart-st.t)/60))} active min`:d.major?'Assault schedule pending withdrawal':`Major assault · Night ${campaignClock({t:d.nextDawn}).day}`,detail:d.clock?campaignWarning(st):d.major?'Current assault in progress. The next schedule updates after withdrawal.':'Smaller raids can come sooner.',location:null});

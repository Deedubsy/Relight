...
export interface CampaignState { version: 1 | 2; opening: 'culdesac-v1'; homeBlock: number; expansion?: ExpansionState }
...
export function rulesetProblem(st: SimState): string {
  ...
  const c = st.campaign;
  if (!c || (c.version !== 1 && c.version !== 2) || c.opening !== CAMPAIGN_RULES.opening || !Number.isInteger(c.homeBlock)

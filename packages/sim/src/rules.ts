...
export function rulesetProblem(st: SimState): string {
  ...
  const c = st.campaign;
  if (!c || ![1,2,3,4,5,6,7,8,9].includes(c.version) || c.opening !== CAMPAIGN_RULES.opening || !Number.isInteger(c.homeBlock)

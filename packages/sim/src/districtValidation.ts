...
export function districtsProblem(st:SimState):string {
  ...
  if(d.station.block===c!.homeBlock||d.station.block===c!.expansion!.station.block||(!st.city?.mapId&&d.workshop.block!==d.station.block)
  ...
  if(d.sources.length!==3||new Set(d.sources.map(s=>s.item)).size!==3||new Set(d.sources.map(s=>s.block)).size!==3
    ||d.sources.some(s=>!rect(s)||s.size!==(st.city?.mapId?3:5)||!['steel','copper','coal'].includes(s.item)||s.block===c!.homeBlock))return 'invalid persistent extraction sources';

...
export interface StationRoute {
 ...
 trams:{machine:Machine;status:'running'|'dwelling'|'disconnected'|'no route'}[];
...
export function stationRoute(st:SimState,id:number):StationRoute|null {
 ...
 const trams=f.machines.filter(m=>m.kind==='tram'&&(seen.has(m.y*f.tw+m.x)||m.manifest?.some(r=>(r.returning?r.origin:r.destination)===id))).map(machine=>{
  ...
  return {machine,status:disconnected?'disconnected' as const:path.length<2?'no route' as const:machine.phase===1?'dwelling' as const:'running' as const};
 });
 const issues:string[]=[];
 ...
 for(const s of stops)if(s.status!=='ready')issues.push(`Stop ${s.machine.id}: ${s.status==='full'?'arrivals full; refused cargo returns':'unpowered; transfers paused'}`);
 for(const t of trams)if(t.status==='disconnected'||t.status==='no route')issues.push(`Tram ${t.machine.id}: ${t.status}; cargo retained`);
 const summary=`${connected.size} stop${connected.size===1?'':'s'} · ${trams.length} tram${trams.length===1?'':'s'} · ${issues.length?issues.join(' · '):'Service connected'}${trams.some(t=>t.machine.manifest?.some(r=>r.returning))?' · Return cargo aboard':''}`;

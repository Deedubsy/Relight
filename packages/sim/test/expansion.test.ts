...
test('old home preview upgrades deterministically without granting a kit; malformed expansion metadata is rejected',()=>{
  const st=createCampaign();delete st.campaign!.expansion;delete st.campaign!.defence;delete st.campaign!.districts;delete st.campaign!.discovery;delete st.campaign!.recruits;delete st.campaign!.turbine;delete st.campaign!.knowledge;st.campaign!.version=1;
  const a=loadState(st),b=loadState(st);assert.equal(a.campaign!.version,9);assert.equal(a.campaign!.expansion!.grantedAt,-1);assert.equal(stateHash(a),stateHash(b));
...
test('upgrading a home-only save preserves the save but labels its old log as incomplete for the new campaign factory',async()=>{
  ...
  try{const loaded=await loadSnapshot('local:old-preview');assert.equal(loaded.state.campaign!.version,9);assert.equal(loaded.logComplete,false);assert.equal(loaded.saved?.state.campaign?.version,1);}finally{if(prior)Object.defineProperty(globalThis,'localStorage',prior);else Reflect.deleteProperty(globalThis,'localStorage');}
});

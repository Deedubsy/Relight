...
test('new entry is empty exploration; explicit legacy entry and state URL do not reinterpret saves',()=>{const s=fresh();assert.deepEqual(s.engineer.inv,{});assert.equal(s.stock.steel+s.stock.copper+s.buffer,0);assert.ok(!s.flow!.machines.some(m=>m.kind==='generator'));assert.equal(parseUrl('').ruleset,'exploration-v2');assert.equal(parseUrl('?rules=legacy-v1').ruleset,'legacy-v1');assert.equal(parseUrl('?state=/old.json').ruleset,undefined);roundtrip(s);});

...
import {
  DEFAULT_CONFIG, SimConfig, protoCalibrated, configHash, generateMap, citySpec, CityPreset, CITY_PRESETS, createState, step, takeEvents,
...
const baseConfig = protoCalibrated({ ...DEFAULT_CONFIG, scatter: true, economy: ECONOMY, walk: WALK });
...
const payload = { hash: HASH, map: MAP, walk: WALK, overrides: JSON.parse(overridesArg), economy: ECONOMY, build: BUILD, react: REACT, hours: HOURS, config, runs, targets: targets.map(([id, what, ok, val]) => ({ id, what, ok, val })) };

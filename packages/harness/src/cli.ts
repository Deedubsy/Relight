/** `npm run experiments [-- --only E2,E4 --seeds 3,4,5 --hours 5 --out docs]`: runs the suite, writes docs/EXPERIMENTS.md and
...
if (args.includes('--nightly')) {
  ...
  for (const e of EXPERIMENTS) {
    ...
    const t1 = Date.now();
    const r = e.run({ seeds, hours, log: s => console.log(`  ${s}`), nightly: false });
    results.push(r);
  ...
  for (const r of results) writeFileSync(join(outDir, 'experiments', `${r.id}.json`), JSON.stringify({ ...r, meta: { seeds, hours } }, null, 1));
  if (!only.length) writeFileSync(join(outDir, 'EXPERIMENTS.md'), toMarkdown(results, { seeds, hours, date: new Date().toISOString().slice(0, 10), seconds, simVersion }));

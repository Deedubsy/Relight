...
export function createInspectionPanel(session:Session,root:HTMLElement,shell:UiShell,controls:HTMLElement[],hooks:{locate(x:number,y:number):void;item(item:Item):void;inventory(x:number,y:number):void;route(x:number,y:number):void}){
 ...
 const upgrades=el('div','row'),recipeSelect=el('select'),chooseRecipe=el('button',undefined,'Set recipe'),attach=el('button',undefined,'Attach carried speed artifact'),detach=el('button',undefined,'Remove speed artifact'),hopper=el('button',undefined,'Upgrade hopper · 20 steel + 10 copper');recipeSelect.setAttribute('aria-label','Machine recipe');upgrades.append(recipeSelect,chooseRecipe,attach,detach,hopper);actions.append(upgrades);
 ...
 attach.onclick=()=>{const m=machine(),item=ITEMS.find(k=>k.startsWith('artifact')&&(session.state.engineer.inv[k]??0)>0);if(m&&item)sendUpgrade({type:'attach',machine:m.id,item});};detach.onclick=()=>{const m=machine();if(m)sendUpgrade({type:'detach',machine:m.id});};hopper.onclick=()=>{const m=machine();if(m)sendUpgrade({type:'hopper',machine:m.id});};
 ...
 return {update(){
  ...
  transfer.hidden=!['chest','tramstop','depot'].includes(info.kind);route.hidden=info.kind!=='tramstop';feed.hidden=true;feed.textContent=info.kind==='generator'?'Load carried fuel':info.kind==='cannon'?'Load carried Shells':'Load carried magazines';feed.disabled=!info.reachable;
  upgrades.hidden=!session.state.campaign;const allowed=m&&isProcessor(m)?recipesFor(m):[];if(recipeSelect.dataset.machine!==String(m?.id)){recipeSelect.replaceChildren(...allowed.map(id=>{const o=el('option',undefined,recipeOf({kind:m!.kind,recipe:id}).name);o.value=id;return o;}));recipeSelect.dataset.machine=String(m?.id);recipeSelect.value=m?.recipe??'shot';}recipeSelect.hidden=chooseRecipe.hidden=!allowed.length;attach.hidden=detach.hidden=!(session.state.campaign?.progression?.sites.some(s=>s.kind==='artifact'&&s.seen)||m?.artifact)||!m||!isProcessor(m)&&!['excavator','pumpjack'].includes(m.kind);attach.disabled=!m||!info.reachable||!!m.artifact||!ITEMS.some(k=>k.startsWith('artifact')&&(session.state.engineer.inv[k]??0)>0);detach.disabled=!m?.artifact||!info.reachable;detach.hidden=detach.hidden||!m?.artifact;attach.title='Adds 10% processing capacity; inputs, power and output space still limit output.';hopper.hidden=m?.kind!=='turret'||!campaignRecruited(session.state,'gunsmith');hopper.title=m?progressionCheck(session.state,{type:'hopper',machine:m.id}):'';hopper.disabled=!m||!!progressionCheck(session.state,{type:'hopper',machine:m.id});
  ...
  if(m?.artifact)inputs.append(p('Speed artifact attached · +10% processing capacity · inputs, power and output space still limit output · 1/1 slot'));

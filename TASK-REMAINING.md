...
- **Authorised?:** Needs go-ahead.
- **Fixed 2026-09-23 (REL-12, with ENM-06):** the premise was checked before it was fixed, and the headline was already gone — REL-74 had given recovery its own branch and stopped the stale countdown printing over a live fight. What was genuinely open was narrower: a warning whose countdown reached 0 before the bodies spawned fell past every branch to "Base under attack", a fight that was not happening; a withdrawal printed `Director.Notice`, which at that moment is as often as not the countdown that brought the raid *in*, in red, over a raid walking away; and `Warning()` returns the small raid when one is live, so an announced major assault vanished from the strip until the small one finished. One rule now decides every line: **the strip describes the state the sim is in, and is red only while something is attacking**, with the director's own sentence printed in exactly one place — the recovery line, where there is no fight to describe. Nothing is lost by that: `HudViewModel.ThreatLine` is the only player-facing reader of `Notice` (checked by grep; `RaidNoticeEvent` feeds only the audio router and the autosave scheduler), and an end-of-raid sentence is said again through recovery moments later. `RaidWarning` gains `AlsoKind`/`AlsoSecondsLeft` — the large raid the line is *not* describing — so a small-raid line carries " · Major assault in N s" (or "under way") beside it; which raid the line and the arrow follow is unchanged. Two new sim tests pin the text and the red flag for all six kinds and both ENM-06 cases. Three staged captures: [evidence/rel-12](Unity/Docs/evidence/rel-12/README.md). No human has played this build.
...
- **Fixed 2026-09-23 (REL-118):** the two once-only guide lines (light hesitation, blind turret) are now retired by time **on screen**, not by the wall clock, so a raid's own Danger rows delay the lesson in the overflow instead of swallowing it. Sim code and two tests; the ranking is untouched. This row's other findings are separate issues: REL-119 (the raid account), REL-116 (the tint's missing line of sight), REL-117 (the sweep) and REL-120 (the Build menu, below) are all done.
- **Fixed 2026-09-23 (REL-120):** the Build menu keeps two card columns wherever the width allows, and the detail pane's stat line stays inside the panel. Found by REL-58 and checked by measuring the running panel, not by arithmetic: under `.compact-ui` — 1280×720, or 1920 with the UI at 150% — the drawer is 680 and the detail pane's 260 + 16 margin left the grid 358px, one 200px card per row with 158px of the row empty. Three style rules, none of them in the card: `.compact-ui .build-detail` is 200 + 8 in `industrial.uss` (grid 426, two columns of 213), `.build-host` grows so a row that can still hold only one host is filled by it, and `#build-panel .panel-sub` wraps — that stat line was 269px of text in a 241px column at 1920 and a 181px one in compact, so it drew past the border at **all three** sizes. One new Play test asserts the columns, no label crossing the inner edge, and — the check that would have caught this — that any line wider than its column wraps or ellipses; it disables `HudController` for the measurement window, because the dock re-derives `compact-ui` from the window's own size every frame. Three captures and the measurements: [evidence/rel-120](Unity/Docs/evidence/rel-120/README.md). No human has played this build.
...
- **Partly fixed 2026-09-23 (REL-62):** the ROWS are re-derived, and the appended ENM-07 is fixed with them. `Simulation.Wrap` raises the transient `SimState.Resumed` (not visited, so neither saved nor hashed) and points the ballistics seam through the new `EnemyTargets.Point`, so a loaded game shoots on its first tick instead of firing one tick of ghost bullets. On its first refresh against a resumed state `DefenceAlertSource` seeds `_raised` from the places carrying a wreck, so the standing "1 structure wrecked" row the player quit with comes straight back; within a session and in a new game the rule is unchanged, because a wrecked wall alone must still raise nothing (U-D-61). Save schema still 9. **Not done:** the guide-line "already taught" flags and the open raid tally are still per session — they cannot be re-derived from the world, so they would need the save schema this issue holds at 9 by design. Three new tests; nobody has played this build. [Evidence](Unity/Docs/evidence/rel-62/README.md).
...
- **Fixed 2026-09-23 (REL-66):** the premise was checked first and was wrong in detail. The sentences did not go to `Debug.Log` only — two on-screen routes existed and both were broken. `FrontEndBootstrap.Collect` put them into the startup notice through an `else if` chain, so at most **one** of the three was shown, and only if and when the player next opened the pause menu; `PauseMenuController.DoLoad` wrote the recovery sentence into a panel it switched away from and closed in the same call, and never carried the other two at all. The balance-data sentence had no on-screen route of any kind. Both routes are retired and replaced by one: `HudViewModel.SayLoad` says each non-empty sentence as its own HUD row — recovery and balance data as Warning, upgrade/region move/heal as Info — in the store's own words, posted through `Teach`, so “once” means once **seen** rather than once timed out. The `LoadResult` instance itself marks it said, so `HudController.Paint` can hand the same load over on every paint and a HUD that wakes after the load still says it. A refusal is not said on the HUD: it changed nothing and the Load screen is already showing it. The log lines stay. Four new tests, three of them loading real files off a real disk in a throwaway temp folder. Evidence: `Unity/Docs/evidence/rel-66/`.

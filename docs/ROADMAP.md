...
Written 2026-09-04 at the end of the D5/D6 rework; **rewritten 2026-09-04 (evening) after prompt B M4; state corrected 2026-09-04 (Step 0 of the standards audit) through the verification pass, b666c68; §4 rewritten from `STANDARDS.md` the same day.** **Refreshed 2026-09-05 at Gate B.** Update at every milestone and gate, alongside `PROGRAMME_STATE.md`. Where this file and `PROGRAMME_STATE.md` disagree, `PROGRAMME_STATE.md` is right and this file is stale.
...
| 4 | **Vertical slice: §11, minutes 0–60** | 6 wk | ◐ **M1–M6 built and verified** (pass of 09-04); pre-M2 controls built; **Gate B played in part 09-05, `proceed — with conditions`**; T9 Absorb Gate B is what is left, and the walkthrough / two-assembler line / stranger test stay waived | `SLICE_REPORT.md` M1–M6 and "## Gate B", `GATE_B.md`, `PROGRAMME_STATE.md` B.1–B.30; commits 042cfe0, 906044c, 7564125, d332464, f7ee1df, 7ba2b9b, f24f383, b666c68 |
| — | **Standards audit** (cross-cutting) | 2–3 d | ✅ run 09-04 | `STANDARDS.md` (matrix, `## Considered`), `STANDARDS_REPORT.md`, D-SA-1–D-SA-5 |
| — | **Layout and readability pass** (new, pre-test) | 2–3 d | ◐ **built 2026-09-05, drawing only (unverified)** — `LAYOUT_PASS_REPORT.md`, `PROGRESS.md` T3 | ten of §2's eleven items built; the lamp cone refused (D-B5-1); STANDARDS rows 4.3, B.3, B.6, C.2 built-for but **unchecked** (human minutes, T5 waived) and the stranger test unrun; D-LP-1/2/3 open |
| — | **Gate B** (human plays the hour) | — | ◐ **played in part 09-05, `verdict: proceed — with conditions`** | `GATE_B.md`; scored in `SLICE_REPORT.md` "## Gate B"; §19's minutes 0–10 only, 10–30 and 30–60 empty; STANDARDS row A.7 **closed**; D-GB-1 decided (the hybrid), D-GB-2/3/4 gate conditions |
...
| **E-rifle at tile scale** | The block-scale E-rifle is in CI (`EXPERIMENTS.md`): steady 5 h ± rifle changes the ammo bill by 0.00 % and loses nothing; in all twelve rescues the block holds with the rifle and without it, so the rifle has never decided a rescue. Owed: the same two questions with M4's bodies on the tiles (an edge red, the belt 90 s away — does the rifle change the outcome, and at what damage-per-round). Not built in M5 or M6. **Done 2026-09-05** (`PROGRESS.md` T4, `SLICE_REPORT.md` "E-rifle at tile scale"): at tile scale the rifle decides a rescue — 48 runs, 7 falls without it and 4 with, 3 saved and 3 delayed, every fall `unfed` and never a shade — and the tile steady table now scores §19's 10 % / 5 % caps (0.50 / 0.69 / 0.06 % and 0.00 %), E-rifle 12 → 14 checks. | Claude Code |
...
| **Gate B** | **Played in part 2026-09-05** (`GATE_B.md`, verdict `proceed — with conditions`): one hands-on session, §19's minutes 0–10 filled and **10–30 / 30–60 empty**, no telemetry read, the burn-off line displaced rather than answered. **A.7 answered and closed** — "Yea the dark is pretty good so far", so D-B5-1 stands. The rework's rifle row is answered (fired, did not matter, D-R1); the walking row is not. Findings: §19's opening ten minutes did not teach what the doc claims, and the hour's one problem was knowing what to do next. Four rows opened, **D-GB-1 decided as the hybrid**. **Still owed: one longer played session that reaches minute 45** — it fills the two empty windows, the burn-off line, the walking row and the first-shade question at once. | **Human** |
| **Absorb Gate B** | Settle with evidence: the steel budget (D-P4-4, D-B1-1), turrets on claimed blocks (D-P4-9), the lattice pip bands (the earlier roadmap cited D-P4-8, which is the start-turret count, superseded 09-04 by D-B1-4), the assembler stand-ins (D-P4-5), the hands / claim minutes / replay ground (D-B6-1 to 3), the half-lit street; fold into code and doc; tag `[play: Gate B]`; recalibrate the tile-level hour if the lattice bands retire; refresh this file. **Re-scoped 2026-09-05 (`PROGRESS.md` T9, unblocked)**: it folds the *partly*-played hour, and it carries D-GB-1's hybrid and the three gate conditions D-GB-2/3/4 into Phase 5. | Claude Code |
...
| 4 Gate B | ~~the hand-lamp sentence (A.7)~~ **closed 2026-09-05**, played dark: the dark reads as the claim's price, D-B5-1 stands | — |
...
- **Gates** — Gate A ✅ (owner-only); **Gate B ◐ played in part 2026-09-05, `verdict: proceed — with conditions`** (minutes 0–10 only; three conditions ride into Phase 5); the **engine gate** in Phase 11.
...
- **Standards** — 52 matrix rows carried a gap on 2026-09-04 (by category: controls 10, logistics 6, information 6, onboarding 2, look 3, environment 1, map 3, meta 6, performance 1, body 4, threat 6, city 4), merging to **46 check rows** where one check closes rows in two categories; build-with rows per phase: layout pass 3, Gate B 1, Phase 5 14, Phase 6 4, Phase 7 1, Phase 8 4, Phase 9 1 (28); add-later rows: Phase 9 2, Phase 10 1, Phase 12 12, Phase 13 3 (18); **closed: 1 of 46** — **A.7, the hand-lamp sentence, closed 2026-09-05** at the partly-played Gate B ("Yea the dark is pretty good so far"): the dark reads as the claim's price and D-B5-1 does not reopen (`GATE_B.md`). The layout pass's four (4.3, B.3, B.6, C.2) are **built for on 2026-09-05 and still unchecked** — human minutes inside the controls walkthrough, which stays waived; 29 items under `## Considered`, 16 of them excluded on purpose by §22 / §23 / the constitution; five decisions open (D-SA-1–5).
...
| What | Where | Blocking? |
|---|---|---|
| **Controls walkthrough** on the reference machine | pre-M2 check list | No longer blocking — **waived 2026-09-05 and Gate B played without it**; still owed, and it carries four STANDARDS rows (4.3, B.3, B.6, C.2) that stay unchecked |
| **Two-assembler line unaided**, ten minutes | M2 DoD | No longer blocking — **waived 2026-09-05**; still owed |
| ~~**Gate B** — play the hour, fill the §19 table, `verdict: proceed`~~ | `GATE_B.md`, `SLICE_REPORT.md` "## Gate B" | **Satisfied 2026-09-05 with conditions.** Phase 5 opens; D-GB-2 (knowing what to do next), D-GB-3 (rifle range) and D-GB-4 (enemy density) ride in as conditions |
| **One longer played session, to minute 45** | `GATE_B.md` §19 table | No — the gate has its verdict. It is the cheapest way to close four open questions at once: §19's 10–30 and 30–60 windows, the burn-off line, the walking share and the first shade |
| **D-GB-2 / D-GB-3 / D-GB-4** — the next-objective problem (rule 8 forbids tutorial screens), rifle range, enemy density | `DECISIONS.md`, `GATE_B.md` | No — gate conditions, settled inside Phase 5. D-GB-4 needs no rule change: §19 allows 10 % shooting where the hour measures 0.50–0.69 % |
| **Art direction lean** — grim-and-quiet (deep blues, warm lamp light) or more saturated | first Phase 12 decision, informs the readability pass palette | No — the readability pass ran 2026-09-05 without asking: flat colour on the palette already in the code, as §2 requires. **D-LP-3** (the soft light falloff) is the one look judgement it does put to a human |
...
| D-P4-4/5/9, D-B1-1, D-B6-1 to 3, the lattice pip bands, the half-lit street | `DECISIONS.md` | No — M6's bot evidence is in (`SLICE_REPORT.md` M6, `E-hour`); settled at T9 Absorb Gate B. **The played hour did not reach them**, so they are settled on bot evidence and a human's word, not on play |

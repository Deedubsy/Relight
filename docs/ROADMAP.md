...
| 7 | the survivor panel as the recipe browser: what makes this, what uses this, who unlocked it (3.5) | — |
...
- **The untagged set** — 44 (Phase 0) → 41 (Phase 3) → 40 (M1) → 37 (M2) → 33 (M3) → 33 (prompt B M1–M6 and the verification pass; every recount B.1–B.15 reads 33 → 33). 21 tile-scale, 12 design inputs. Gate B's `[play]` tags are the next expected drop.
...
- **Generated-report freshness** — partial. `calibration.md` and the snapshot carry the config hash (03328db7 / 68d07000); `snapshot:check` fails CI on a mismatch; calibration is regenerated in CI and reported, not gated; no report carries its source commit. The full rule the earlier roadmap claimed ("every generated report carries its source commit and config hash; CI fails on mismatch") is not built — it goes to `DEFERRED.md` with the next verification pass as its owner.

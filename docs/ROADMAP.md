...
- [ ] 1.. Layout and readability pass — owner: Claude Code — evidence lands in: SLICE_REPORT.md — ticked 2026-09-04: `SLICE_REPORT.md` "Economy fix" Built (unverified: no human check yet) — un-ticked 2026-09-05 (economy-fix task Step 5, `ECONOMY_FIX_REPORT.md` §7): the tick was a self-report; the human check of the pass is line 5, so this line stays the first unticked line until a human has looked
- [ ] 2.. E-rifle at tile scale (the rescue run and the steady run) — owner: Claude Code — evidence lands in: EXPERIMENTS.md — ticked 2026-09-04: `EXPERIMENTS.md` "E-rifle-tile-steady" and "E-rifle-tile-rescue" (12 / 12 checks, full run of 13 experiments green) — un-ticked 2026-09-05 (economy-fix task Step 5): on the decided minutes (D-HOUR-1, north at 40) E-rifle is 10 / 12 — north falls in the steady run rifle off and on, and the 600 s edge rescue on seed 5 at 35:00 knocks the engineer down (`EXPERIMENTS.md`, `ECONOMY_FIX_REPORT.md` §6)
- [x] 3. The economy fix from M6 — owner: HUMAN FIRST (write rows D-P4-4, D-B1-1, D-P4-7, D-P4-8, D-B5-4 in DECISIONS.md), then Claude Code builds — evidence lands in: DECISIONS.md, then SLICE_REPORT.md — ticked 2026-09-04: the human wrote the five decisions in the chat message (not in the file); the rows carry `decided by / on / via` that message; built, `SLICE_REPORT.md` "Economy fix" — re-ticked 2026-09-05 (economy-fix task Steps 1–3): the nine rows D-P4-4, D-B1-1, D-P4-7, D-P4-8, D-B5-4, D-B1-4-rider, D-ENGINE-1, D-INSERTERS-1, D-HOUR-1 are decided in `DECISIONS.md`, the eight disagreements are fixed and `docsync:check` is green (`ECONOMY_FIX_REPORT.md` §1–§3)
- [ ] 4.. Re-run E-hour until the hour has no block falling on seeds 3, 4 and 5 — or rewrite §11 to what the economy can afford — owner: Claude Code — evidence lands in: SLICE_REPORT.md — ticked 2026-09-04: §11 rewritten (two claims, north at 60–75); no fall, no brownout, steel ≥ 30 on seeds 3 / 4 / 5, `SLICE_REPORT.md` "Economy fix" Measured — un-ticked 2026-09-05 (economy-fix task Step 4, `B-M6-hour-2`): the earlier tick was measured on north 65 / steel2 12; on the decided minutes (north 40, steel2 15) the hour is not no-fall — north falls at 48:54–52:43 after the 40:01→45:00 brownout and chest steel is zero at 15:02–15:04 on every seed; the two-claim variant (north 65) holds the hour but steel still reaches zero (`SLICE_REPORT.md` "M6 re-run after the economy fix", `docs/experiments/E-hour.json`). The human decides D-P4-10 and D-HOUR-2 before this line can be ticked

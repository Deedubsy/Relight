...
- [x] 1. Layout and readability pass — owner: Claude Code — evidence lands in: SLICE_REPORT.md — ticked 2026-09-04: `SLICE_REPORT.md` "Economy fix" Built (unverified: no human check yet)
- [x] 2. E-rifle at tile scale (the rescue run and the steady run) — owner: Claude Code — evidence lands in: EXPERIMENTS.md — ticked 2026-09-04: `EXPERIMENTS.md` "E-rifle-tile-steady" and "E-rifle-tile-rescue" (12 / 12 checks, full run of 13 experiments green)
- [x] 3. The economy fix from M6 — owner: HUMAN FIRST (write rows D-P4-4, D-B1-1, D-P4-7, D-P4-8, D-B5-4 in DECISIONS.md), then Claude Code builds — evidence lands in: DECISIONS.md, then SLICE_REPORT.md — ticked 2026-09-04: the human wrote the five decisions in the chat message (not in the file); the rows carry `decided by / on / via` that message; built, `SLICE_REPORT.md` "Economy fix"
- [x] 4. Re-run E-hour until the hour has no block falling on seeds 3, 4 and 5 — or rewrite §11 to what the economy can afford — owner: Claude Code — evidence lands in: SLICE_REPORT.md — ticked 2026-09-04: §11 rewritten (two claims, north at 60–75); no fall, no brownout, steel ≥ 30 on seeds 3 / 4 / 5, `SLICE_REPORT.md` "Economy fix" Measured
...
- Step 5 of the guardrails landed (`constants.ts` is the one source and `docsync:check` reports every disagreement), but six recorded disagreements between the calibration config / `firsthour.ts` and the tile code stand until a human settles each (`GUARDRAILS_REPORT.md` §5 items 2–7); `docsync:check` is red by design until then.

...
Written 2026-09-04 at the end of the D5/D6 rework; **rewritten 2026-09-04 (evening) after prompt B M4; state corrected 2026-09-04 (Step 0 of the standards audit) through the verification pass, b666c68; §4 rewritten from `STANDARDS.md` the same day.** **Refreshed 2026-09-05 at Gate B (passed).** Update at every milestone and gate, alongside `PROGRAMME_STATE.md`. Where this file and `PROGRAMME_STATE.md` disagree, `PROGRAMME_STATE.md` is right and this file is stale.
...
| — | **Gate B** (human plays the hour) | — | ✅ **passed 09-05, `verdict: proceed — with conditions`** | `GATE_B.md`; scored in `SLICE_REPORT.md` "## Gate B"; §19's minutes 0–10 recorded, 10–30 and 30–60 not; STANDARDS row A.7 **closed**; D-GB-1 decided (the hybrid), D-GB-2/3/4 gate conditions |
...
| ~~**Gate B**~~ | **Done 2026-09-05** (`GATE_B.md`, verdict `proceed — with conditions`, closed as fully tested): played hands-on, §19's minutes 0–10 recorded and **10–30 / 30–60 not**, no telemetry read, so the hour's shares stay the bot's. **A.7 answered and closed** — "Yea the dark is pretty good so far", so D-B5-1 stands. The rework's rifle row is answered (fired, did not matter, D-R1); the walking row is not. Findings: §19's opening ten minutes did not teach what the doc claims, and the hour's one problem was knowing what to do next. Four rows opened, **D-GB-1 decided as the hybrid**. | **Human — done** |
| **Absorb Gate B** | Settle with evidence: the steel budget (D-P4-4, D-B1-1), turrets on claimed blocks (D-P4-9), the lattice pip bands (the earlier roadmap cited D-P4-8, which is the start-turret count, superseded 09-04 by D-B1-4), the assembler stand-ins (D-P4-5), the hands / claim minutes / replay ground (D-B6-1 to 3), the half-lit street; fold into code and doc; tag `[play: Gate B]`; recalibrate the tile-level hour if the lattice bands retire; refresh this file. **Re-scoped 2026-09-05 (`PROGRESS.md` T9, unblocked, and the only Phase 4 task left)**: it folds the played hour, and it carries D-GB-1's hybrid and the three gate conditions D-GB-2/3/4 into Phase 5. | Claude Code |
...
- **Gates** — Gate A ✅ (owner-only); **Gate B ✅ passed 2026-09-05, `verdict: proceed — with conditions`** (closed as fully tested; §19's later windows unrecorded; three conditions ride into Phase 5); the **engine gate** in Phase 11 is the only one left.
...
| ~~**Gate B** — play the hour, fill the §19 table, `verdict: proceed`~~ | `GATE_B.md`, `SLICE_REPORT.md` "## Gate B" | **Done 2026-09-05, with conditions.** Closed as fully tested by the owner. Phase 5 opens; D-GB-2 (knowing what to do next), D-GB-3 (rifle range) and D-GB-4 (enemy density) ride in as conditions |

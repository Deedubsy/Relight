...
**Current phase: 4, reopened by the rework (D5 the engineer on foot, D6 the street-first city; `REWORK_REPORT.md`, 2026-09-04). The lattice slice M1–M3 (`SLICE_REPORT.md`) is not scored at Gate B; the slice is being rebuilt from `docs/relight-prompt-B-vertical-slice.md`: D-R1/D-R2/D-R3 taken by recommendation on the human's `go` (2026-09-04), prompt B M1 "Ground and the engineer" built the same day. prompt B M2 "Flow on the faces", M3 "Defence on the segment", the before-M4 gate (D-P4-7 settled, D-B3-4 built) and M4 "Threat, and the rifle" followed on 2026-09-04, M5 "Light" and M6 "The hour, §11 as rewritten" on 2026-09-04 (both unverified). Prompt B's six milestones are built. Next: the verification pass when the human schedules it (M5 and M6 have never run; `E-hour` fills the M6 timeline), then Gate B — a human plays the hour and scores §19 with its two new rows; D-P4-4/5/7/8/9 settle from that hour. **Working mode from 2026-09-04 (user): a `go` is code changes and a light review only; the scripted checks, experiments, calibration and the soak form a separate verification pass the user schedules, and milestone numbers are marked *unverified* until it has run** (`CLAUDE.md`).** D-P3-9/10/11 were made by their recommendations on that `Go` (sessions in parallel, C3/C4 as locked, C8 lock accepted). PR #1 (`phase-1` → `main`), PR #2 (`phase-2` → `phase-1`) and PR #3 (`phase-3` → `phase-2`) are open, unmerged; Phase 4 is PR #4 (`phase-4` → `phase-3`). Phase 1's human five-minute smoke test and the Gate A sessions are still the user's.
...
### B.11 M5 Light (2026-09-04, `SLICE_REPORT.md` "Prompt B M5") — unverified
...
- **Unverified**: no check, experiment, calibration or soak has run (the working mode of 2026-09-04); the report lists what the pass should measure.
...
### B.13 M6 The hour, §11 as rewritten (2026-09-04, `SLICE_REPORT.md` "Prompt B M6") — unverified
...
- **Unverified**: no check, experiment, calibration or soak has run (the working mode of 2026-09-04) — the bot has not played an hour; the report's timeline is what the code schedules and the findings are E-hour's to write.
...
- **Doc**: no `[sim: B-M6-hour]` tag placed — nothing has been confirmed; one changelog line says where the tags land after E-hour runs. `SLICE_REPORT.md` rewritten from the top: the GAME-ASSUMPTION register and Gate B's two rows at the top. `DEFERRED.md` "Re-read at prompt B M6". D-B5-1/2/3 taken by recommendation (a) on this `go`; D-B6-1 (the hands from the pockets), D-B6-2 (the claim minutes), D-B6-3 (the replay's ground) open; D-P4-4/5/7/8/9 carry a recommendation each, settled by the human after the played hour.

### B.14 Untagged recount at prompt B M6

33 → **33**. M6 confirms no doc number (unverified): no tag placed, no number moved, no row left or joined the untagged set. §26: with the hour bot the slice's ten gate criteria are all measurable by E-hour but the three §19 asks (new unlock, current problem, memorable moment) and the burn-off's line, which are the human's; 5 of 10 measured once the verification pass runs.

...
**Current phase: 5 — the factory, complete, opened 2026-09-05 by `PROGRESS.md` T10 and unblocked the same day by T11 (`PHASE_5_REPORT.md`, sections 5.1–5.4 below). Nothing is built yet: the opening was constitution rule 12's comparison and T11 was the human answering the fourteen rows it raised. The first build task is T12 — M1.**
...
## Phase 5 — the factory, complete (opened 2026-09-05)

**Task list: `docs/PROGRESS.md` (T10–T18).** **Status: open, nothing built, and no longer blocked. T10 done 2026-09-05 (the opening, `PHASE_5_REPORT.md`, rule 12); T11 done 2026-09-05 — the human decided all fourteen rows in one message ("Go with your recommendation for all"), so `DECISIONS.md` holds no `open` row and every build task is clear. **T12 — M1: recipes, rubble, the rail yard's coal at ~700 and hour one re-scoped to 75 minutes — is the next task.** Sections 5.1–5.4 below.**

### 5.1 The opening (T10, 2026-09-05) — doc only, rule 12

Phase 5 is §12–§14 in full: the eight non-Shot recipes made by real machines, the §13 machine list with footprints, belts / undergrounds / splitters / one inserter / chests / the Depot, trams and the driven truck, rubble as finite typed ore with visible depletion, power complete with proportional brownout, and slots as real lot geometry. Six weeks. It adds **no new system**: every machine it builds serves the front, the found tech or automated combat that already exist, so §26 stays at three systems and complexity 5 / 10.

**The rule-12 read is the opening's substance.** Seven rules in the phase's line are in neither `RELIGHT-design.md` nor a decided row, and are **listed and not built** as `DECISIONS.md` rows **D-P5-1 … D-P5-7**:

- **D-P5-1 — "the dimming on the map view."** The doc says the opposite twice: §14's *"Lights do not dim: a Lamp, Floodlight or streetlight is lit at any throttle above zero and dark only on a dead grid"*, and the brownout's legibility argument, which is the HUD bar and machines visibly slowing. §4's map-view vocabulary has ten entries and no throttle state; §24 risk 4's icon marks a fall, not a throttle.
- **D-P5-2 — "the §13 machine list … and nothing outside it."** §13 is the whole game's list: Cannon, Barricade, Mixer, Pumpjack, the six facilities and the Line truck garage unlock in Phases 6–8. Concrete is the sharp case — §12 lists it among the eight intermediates Phase 5 owes and its only machine is found tech.
- **D-P5-3 — §14's two-lane belts against `flow.ts`'s one.** `STANDARDS.md` 2.1 routed this here by name ("the human settles §14 vs `flow.ts` when Phase 5 opens"), and it is the one standards row where the baseline diverges — Factorio alone of seven has two lanes. It decides how much of M3 exists.
- **D-P5-4 — the blueprint file.** §14 and §19 say "blueprints and copy-paste from minute one"; `PHASES.md` puts blueprints in Phase 8 and Phase 7's dealbreaker-1 row makes them a found unlock (GA-EF-3); the DoD needs one for the bot. Three documents, three answers.
- **D-P5-5 — where the filter lives.** STANDARDS 2.5 is a build-with row for this phase and §13 carries no filter sentence; D-INSERTERS-1 decided one inserter, which makes the row's own note ("which of the three inserters filters") stale.
- **D-P5-6 — the tram route on the map view.** §4's fixed vocabulary draws trams as moving rectangles and belts as dashed lines; the *route* between two stops is not in it, and STANDARDS 2.11 asks for it.
- **D-P5-7 — the experiment names.** `E10` already names the retired Python bloom-cadence run, cited in §25 item 1, C8, D-P3-5 and D-P3-11, so a `[sim: E10-…]` tag would be ambiguous in the one section that carries one. Recommend `E-chain` / `E-coal` / `E-tram`, the tile-era convention.

**Two sentences the doc plainly settles were corrected in `PHASES.md`** rather than asked, on the guardrails task's precedent and recorded in that file's footnote: "six inserters per stop" → **"up to six inserters a stop (§13)"** (§13 and §14 both say *up to* 6, one per tile of the stop's long sides; a fixed six pre-answers E12, which exists to measure exactly that), and "oil from the Refinery only" → **crude from the Pumpjack, the Refinery its unlock and its only consumer** (§12). Four clauses that are not settled carry `[open: …]` tags in that file instead of edits.

**One design-doc edit, one changelog line**: §13's Inserter row now carries D-INSERTERS-1's answer — one inserter, no tier ladder, a faster one later as found tech only if the chain-throughput experiment finds a chain that cannot reach its §12 rate. That row's own `doc sentence` column said the sentence lands "§13 when Phase 5 opens", so this is a decided row being written down, not a new rule.

**One finding against an existing row.** `DECISIONS.md` **D-P4-12** said "two constants are missing and neither is in the design doc"; §12's raws table in fact reads *"Coal | rail-yard rubble (direct, ~30k per block)"*, which is the row's own option (a) with a quantity **43× its recommended ~700**. The premise is corrected and the row is left open: §12's number carries no `[sim: …]` tag, and taking it makes west's claim the answer to coal for the rest of the game rather than a top-up for hour two.

**The question `PROGRESS.md` addressed to this opening is answered.** §11's enclosure and the Electricians are **the shape of hour one, not a hole in it**: both beats are gated on north, north is at 65 by D-P4-10, and §11's own paragraph says so ("past the hour on purpose"). What is left is that §11's "30–60 min" heading now describes minute 65 — either the beats get their own heading, or the hour is re-scoped to 75 minutes, which `E-hour-north` already measures green (Held 65:29–65:34, standing to 75:00 on every seed). A human's call.

**The check the phase adds**: `docsync:check` learns, in M1, that every `[play: <gate>]` tag must name a gate whose evidence file exists and carries a verdict. It closes the process finding T9 parked here — a tag naming a gate that has not happened cannot be checked by anything in the repo today — and it is the one piece of Phase 5 work with no rule-12 question in front of it, because it invents no game rule.

**Milestones (proposed, not started)**: M1 recipes and rubble; M2 placement and the command layer (with every front edge physical, and undo/redo); M3 belts complete; M4 power and the panels (with D-GB-2's next-objective line); M5 trams and the driven truck; M6 slots, the blueprint file and the DoD. `PROGRESS.md` T12–T17, every one blocked on T11. Gate B's three conditions (D-GB-2/3/4) are resolved inside the phase by the verdict; **D-GB-1's hybrid has no milestone and needs a phase from the human** — the opening recommends Phase 6, because it changes the loop rather than completing the factory.

Cheap checks green. Nothing measured: the opening ran no experiment and wrote no code, so it has no number to mark *unverified*.

### 5.2 Untagged recount (rule 2, at the phase opening)

**33 → 33.** The opening adds no number to `RELIGHT-design.md`. Its one design-doc edit — §13's Inserter sentence — states a decided row's answer and carries no constant. §26 unchanged: **three systems, complexity 5 / 10**. Phase 5 completes the first of the three (belts, inserters, machines) and adds no fourth, which is the point of rule 2's recount here; the next thing that raises it is D-GB-1's expedition, on whatever phase builds it.

### 5.3 The decisions (T11, 2026-09-05) — doc only, no code

**T11 was a human task and was written in on the human's instruction**, the message *"Go with your recommendation for all"*, which answered the four groups `PHASE_5_REPORT.md` and the previous report named. Every row was taken on **its own recommendation** and carries the same provenance triple (rule 11): `decided | Daniel | 2026-09-05 | the chat message of 2026-09-05 ("Go with your recommendation for all"), taking this row's recommendation`.

**Fourteen rows decided, and `DECISIONS.md` now has no `open` row at all** — only D-GB-2, D-GB-3 and D-GB-4 remain `open (gate condition)`, which is what Gate B's verdict made them.

| row | taken | where it lands |
|---|---|---|
| D-P5-1 | (a) **no map-view dim** — the HUD bar and the machine panel carry the brownout read | no doc edit; §14's "lights do not dim" stands. M4 |
| D-P5-2 | (a) every §13 row whose unlock is `start` or a Phase 5 system; nothing outside §13 | `PHASES.md` reworded. M2 |
| D-P5-3 | (a) **one lane** | §14 rewritten + changelog; `STANDARDS.md` 2.1 closed as a deliberate divergence. M3 |
| D-P5-4 | (a) the word splits: harness blueprint **file** in Phase 5, in-game blueprint Phase 7's unlock / Phase 8's build | §14, §19's tedium audit, §24 risk 1 rewritten + changelog; `STANDARDS.md` 1.11 and dealbreaker 1 closed. M6 |
| D-P5-5 | (a) a **per-inserter filter setting**, one item kind, off by default | §13's Inserter row + changelog; `STANDARDS.md` 2.5 closed. M3 |
| D-P5-6 | **(c)** the route drawn only while a stop is selected | no doc edit — §4's vocabulary stays at ten entries. M5 |
| D-P5-7 | (a) `E-chain`, `E-coal`, `E-tram` | `PHASES.md` corrected; `E10` stays the retired bloom run in §25, C8, D-P3-5, D-P3-11 |
| D-P4-2 | the stand-in through M2; M1's rubble model sets units per tile against §12's 300 | M1, measured by `E-coal` |
| D-P4-3 | the 160-tile placeholder stays inert; facility placement is Phase 9's | M1 — **and M1's report must say which half it built**, because `E-coal` / `E-tram` may want the deposits live |
| D-P4-12 | (a) a `coal` rubble kind on rail-yard / industrial lots, **~700 a lot** | §12's raws table rewritten from ~30k + changelog. M1 |
| C5 | coal ~700 (already in the doc body), steel the named placeholder M1 sizes | no doc edit |
| D-B2-1 | (a) stands for the slice; **(b) is what Phase 5 builds** — a workbench recipe with a craft time, priced by M1's table | M1 + M2 |
| D-GB-1-rider | **Phase 6 builds D-GB-1's hybrid**, as its own milestone and its own doc-edit pass | `PHASES.md` Phase 6 |
| D-HOUR-3 | **hour one re-scoped to 75 minutes**; north's claim a scored check with its own DoD line | **M1** — §11's minute table is generated from `constants.ts` `HOUR`, so it is a code change first |

**The one row that overrode a design-doc number rather than filling a blank is D-P4-12.** §12's raws table read *"Coal | rail-yard rubble (direct, ~30k per block)"*, untagged; the row's recommendation was ~700 and the row put both in front of the human before it was signed. §12 now reads ~700 with a changelog line. At ~30k west's claim would answer coal for the rest of the game rather than topping up hour two, so `PROGRESS.md` names it as the sentence to reopen if that was the intent, and `E-coal` measures it at M1.

**Three rows Claude took `provisional` under rule 14, not the human**, because they are named by Phase 5 milestones but were not in T11's list: **D-B2-2** (slots stay `floor(area / 600)` now, dropped at M6), **C3** (unchanged), **D-SA-2** (a string table in Phase 5). They are attributed to Claude Code, 2026-09-05, and T13 and T17 carry them in their blocked-by lines so they surface before the work reaches them.

**One task line was corrected.** T15 was blocked on "D-GB-2's form". Gate B passed *with* D-GB-2 as a condition to be **resolved inside Phase 5**, so M4 resolves it rather than waiting on it; the row's recommendation is the rule-8-legal HUD next-objective line first, then re-ask. A quest system remains the human's to take, and takes constitution rule 8 with it.

**Five design-doc sentences changed, four changelog lines** (§14's belts; §14 + §19 + §24's blueprints, one line; §13's Inserter; §12's coal). No code file changed: every decision's build belongs to a milestone, and T12 (M1) is the first.

### 5.4 Untagged recount (rule 2, after T11)

**33 → 33.** T11 changed five design-doc sentences and added no number that is not a decided row's: §12's coal is D-P4-12's ~700, §14's one lane, §13's filter, §14 / §19 / §24's blueprint gate and §11's 75 minutes are all tagged to rows. §26 unchanged: **three systems, complexity 5 / 10**.
...
### B.29 Gate B — passed, verdict `proceed — with conditions` (2026-09-05, `docs/GATE_B.md`, `SLICE_REPORT.md` "## Gate B") — **human**

`docs/PROGRESS.md` **T8**, the gate `ROADMAP.md` names as the one Phase 5 does not open without. Daniel played the hour hands-on, answered the gate's four questions in his own words, and **closed the gate as fully tested on 2026-09-05**; the answers are quoted in `GATE_B.md` and the same verdict is in the slice report, which is the evidence file `ROADMAP.md` points at. **T8 is `done` and nothing is owed back to the gate.** Nothing was built by the gate and no constant moved.

- **Where the gate's numbers come from** (rule 11's provenance, rule 13's "built means measured"). §19's first-hour table has **minutes 0–10 filled with the player's words and 10–30 and 30–60 empty** — not recorded, and nothing inferred into them; no telemetry panel was read, so the hour's own numbers (shooting share, danger share, walked share) stay **the bot's**, tagged `[sim: …]` and never `[play: Gate B]`. The three other human tasks — the controls walkthrough (T5), the two-assembler line (T6) and the stranger test (T7) — **stay waived**.
...
- **What the gate leaves to Phase 5, as observations rather than debts**: the burn-off question, which was asked and displaced by the loop redesign rather than answered; **the first shade**, where §11 / E3 say 32–47 and six bot hours say never — `E-hour`'s open finding, neither confirmed nor contradicted by play; and the walking share, still the bot's 4.6–5.2 %. Any later session that reaches minute 45 answers all of them at once; none of them holds anything up.
...
### B.31 Absorb Gate B (2026-09-05, `PROGRESS.md` T9 done, `SLICE_REPORT.md` "Absorb Gate B") — **doc only, no code**

The last Phase 4 task. It folds the played hour into `RELIGHT-design.md`, records D-GB-1's hybrid as a decision the doc carries but has not built, attaches the gate's evidence to the rows that were waiting on it, and hands D-GB-2 / D-GB-3 / D-GB-4 to Phase 5 as the verdict's three conditions.

- **No code file changed, and that is the finding, not an omission.** The gate produced no constant: §19's 10–30 and 30–60 windows were never recorded, no telemetry was read, and the four questions it opened are three asks the design doc does not contain plus one decision whose build is Phase 5 / 6 work. Building D-GB-1's expedition, D-GB-3's range or D-GB-4's density now would be building from a prompt rather than from a decided doc row — constitution rule 12 — so T9 stops at the doc. `packages/sim`, `packages/game` and the config hash are untouched; the experiment suite is the same 13 experiments, 0 failing checks that B.27 left green.
- **Two `[play: Gate B]` tags were already in the doc before the gate was played** (commits 27cc9b0 and 906044c placed them in advance), and both were provenance claims the played hour does not support as written. Both are now correct:
  - **§4's body sentence.** The blanket tag is split. WASD is what the hour reached — *"movement is good, it's kind of intuitive but needs more development time"* — and that clause keeps `[play: Gate B]`; sprint, dodge and "the game is completable without any of them" were never exercised and keep only `[sim: B-M1-body]`. The 9-tile range with no advantage over a turret is named as the one clause play argued with (**D-GB-3**), and it stands as written until that row is decided.
  - **§19's 10 % shooting cap.** The `[play: Gate B]` tag is **removed**: the gate read no telemetry panel, so there is no player shooting share and the cap cannot carry a play tag. The bot's 0.50 / 0.69 / 0.06 % shooting and 0.00 % danger **[sim: E-rifle-tile]** are stated in its place, and D-GB-4's ask is named against that headroom.
- **§19's first-hour test now carries the finding against itself.** Neither of minute 0–10's two rules ("blocks bloom"; "ammo is made from rubble") was named back; movement was, which the test does not count as a rule; and the session's one problem was not knowing what to do next (**D-GB-2**, against constitution rule 8, so the rule-8-legal forms are listed first). The later windows are marked untested rather than contradicted.
- **§10 carries D-GB-1's hybrid as decided-and-not-built.** The three loops stay exactly as written and every measured number stands; what is added is the hybrid's own content — powering and lighting the next area as an expedition the engineer walks, and an area lighting up as a threat trigger beside the wake bloom — with the statement that §5, §11 and §18 do not move until the milestone that builds it, with a changelog line under rule 1.
- **§19's walking share stops implying a human number.** The Hands test said "a player's share is M6's number"; M6 measured the bot (4.6–5.2 % at tile scale) and Gate B read no telemetry, so the sentence now states the bot's share as the bot's and says the player's is still unmeasured.
- **§25 gains open questions 19, 20 and 21** — the next-objective problem, rifle range, and enemy density at the player — which is where the three gate conditions live in the spec rather than only in `DECISIONS.md`. The Changelog preamble now says what a `[play: Gate B]` tag means beside `[play: Gate A]`: a **witnessed statement from one played hour, never a measured share**.
- **Four `provisional` rows were given the gate's evidence, and none was decided** (rule 7 — the constants are the human's). **D-R1**: the rifle's play half came back and agrees with the bots, and A.7's answer closes option (b), the hand lamp, so **(a) is the only live option left and the recommendation is now to take it**. **D-P4-5**: the condition on its recommendation — the block-level assembler button stays through Gate B so the played hour is one model — is met, so "make every assembler physical" is unconditioned. **C1 and C2** both named Phase 4's human hour as the first place they could move; it was played, but with no telemetry and no claim minutes taken, so neither moved and both stay `provisional` on the sim's numbers.
- **What T9 deliberately did not do.** It did not infer a number into §19's empty windows; it did not move a `[sim: …]`-tagged sentence to suit the hybrid; it did not mark D-GB-2 / 3 / 4 as anything but `open (gate condition)`; and it did not touch §5's claim rule, which is exactly what D-GB-1's option (c) was chosen to preserve.

### B.32 Untagged recount at Absorb Gate B

33 → **33**. T9 added no number to `RELIGHT-design.md`: the seven edits are a provenance split, a tag removal, a finding, a decision record, three open questions, a changelog preamble sentence and one stale clause corrected (§19's walking share was described as "M6's number"; it is the bot's, and no human number exists), and the only figures they carry (0.50 / 0.69 / 0.06 % and 0.00 %) arrive already tagged `[sim: E-rifle-tile]` from B.27. One tag left the doc (`[play: Gate B]` on §19's cap) and one was narrowed (§4's), which moves provenance rather than the untagged set. §26 unchanged: three systems (belts/inserters/machines, the front, found tech), complexity **5/10** — D-GB-1's expedition raises it on the milestone that builds it, not here.

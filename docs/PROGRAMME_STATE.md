...
**Current phase: 4, reopened by the rework (D5 the engineer on foot, D6 the street-first city; `REWORK_REPORT.md`, 2026-09-04). The lattice slice M1–M3 (`SLICE_REPORT.md`) is not scored at Gate B; the slice is being rebuilt from `docs/relight-prompt-B-vertical-slice.md`: D-R1/D-R2/D-R3 taken by recommendation on the human's `go` (2026-09-04), prompt B M1 "Ground and the engineer" built the same day. prompt B M2 "Flow on the faces", M3 "Defence on the segment", the before-M4 gate (D-P4-7 settled, D-B3-4 built) and M4 "Threat, and the rifle" followed on 2026-09-04. Next: on `go`, prompt B M5 "Light".** D-P3-9/10/11 were made by their recommendations on that `Go` (sessions in parallel, C3/C4 as locked, C8 lock accepted). PR #1 (`phase-1` → `main`), PR #2 (`phase-2` → `phase-1`) and PR #3 (`phase-3` → `phase-2`) are open, unmerged; Phase 4 is PR #4 (`phase-4` → `phase-3`). Phase 1's human five-minute smoke test and the Gate A sessions are still the user's.

...
## 9. Step 7 — the three blockers written and built; main is green (2026-09-05)

On the instruction "Do the blockers" the three rows §8.5 left for the human were written into `docs/DECISIONS.md` as **decided** — `decided by` Daniel, `on` 2026-09-05, `via` the chat message of 2026-09-05 ("Do the blockers"), each taking that row's own recommendation — and then built. T1's third clause, "make main green", is met.

### 9.1 What was decided, and what each one changed

| row | taken | built |
|---|---|---|
| **D-HOUR-2** | (a) — the second steel Excavator at minute 12 | `constants.ts` `HOUR` step `steel-2` 15 → **12**; §11's generated minute table regenerated by docsync; GA-EF-1 (the provisional 12:00) is now a decided row rather than an assumption |
| **D-P4-10** | (a) — north at 60–75 with west's coal made real first. **Half built** | `constants.ts` `HOUR` step `claim-north` 40 → **65**, past the hour on purpose; `HOUR_END.held` 4 → **3**; `hour.ts` expects north's claim, the HQ's white border and the Electricians only from `northAt`; `firsthour.ts`'s comment says the `t < 3600` loop never adds north's substation. **The other half was not built** — see §9.3 |
| **D-P4-9** | the block-level hopper stays through Gate B, so §11's two carried turrets go | `hour.ts` lost the carry step and with it `carryTurrets`, `turretSpots`, `idleTurret`, `hqTurrets` and `HourBot.carried`; `flow.ts`'s `pickUpItems` comment no longer cites them; `hour.test.ts`'s minute-45 test rewritten for two claims |

The light review of the diff also found and fixed one defect of its own: `E-hour-stock`'s header was thirteen hardcoded labels (`0:00`…`60:00`) against twelve columns of data — the chest is sampled at t = 0…3540 — so every value read one five-minute step late. The header is now derived from the report's own sample times.

### 9.2 Measured — the full suite, 2026-09-05

**`npm run experiments`: 13 experiments, 231 s, 0 failing checks.** E-hour is **10 / 10** (it was 6 / 10 in §8.4) and E-rifle is back to **12 / 12** (it was 10 / 12; the seed-5 knockdown in the 600 s edge rescue does not recur). `npm run snapshot:check` matches at config `0176f61d`; `npm test` 121 / 121; typecheck, lint and `docsync:check` green.

| what §8.3 measured red | now |
|---|---|
| chest steel 0 at 15:02–15:04, every seed | **30 at 12:01–12:02**, every seed — the curve's minimum, D-P4-4's check green |
| 262–315 s of brownout (exactly 300 s in the runs that survive), every run | **0 brownout seconds, 0 refusals**, all six runs |
| north falls; 3 of 6 runs lose a block | **0 falls across 6 runs**; 3 Held, HQ held, 6 turrets, 4 Generators, 7 Excavators, 3 Assemblers, 519 line magazines, every run |
| — | walking **2.2–3.8 %** of the hour against §19's 15 %; claim walk-overs **9–14 s** against the 60 s rule |

**`E-hour-north` (measured, not scored) is the result worth naming.** Carried to 75:00 with north claimed at `constants.HOUR`'s 65:00, north is **Held at 65:29–65:34 and never falls on any of the three seeds — 0 falls, 4 Held at 75:00** — where the same variant fell at 72:45–74:38 on every seed while §11 carried two turrets onto it. Removing the turrets is what saved it: a physical turret on a claim takes its edge off the ring feed (`hookSyncEdges`) and swaps a ring-fed 100-round hopper filled from a buffer for a hand-fed 50, and one engineer cannot keep three of those fed. On the block-level hopper north now rides out the **510–557 s** brownout that the hand-fed turrets did not. The Generators still run dry at **67:51–68:01** and the chest at 75:00 reads 1520–1524 steel / 397 copper / **0 coal**.

The hour's coal has moved too, and the design doc was wrong about it: the chest's coal runs out **for good at 56:00** and reads **0 at minute 60** on every seed, not the 2 §11 claimed. §11's prose was corrected (four sentences), along with the second steel Excavator's minute, north's minute and the carried turrets.

### 9.3 What could not be built, and the new row it asks for

**D-P4-10 (a) is half a decision the code can keep.** Its second half — "west's coal made real", a physical Excavator and belt on west's rubble at the claim — has nowhere to stand: `rubbleOf` returns stone (civic), copper (residential), steel (industrial) or null (outskirts) and there is **no coal rubble kind at all**; coal exists only as an outskirts deposit. West is a rail yard, so it reads `ind`, and a physical Excavator on its lot would dig **steel**. Two constants are missing — what a rail-yard lot's coal is, and how much of it there is — and neither is in the design doc or in a decided row, so under constitution rule 7 they are the human's. The work is deferred (`DEFERRED.md`) and the constants are asked as **D-P4-12**, which blocks nothing: the hour is green without it, and what it buys back is the Generators running dry at 67:51–68:01 in the 75-minute variant.

Also deferred, from D-P4-9's own recommendation: **every front edge physical — a claim laying its segment's turrets from stock — after Gate B.** The four functions it needs were deleted rather than hidden; `DEFERRED.md` names commit `6694b71` as the last one that has them. It cannot be built until the ring feed keeps feeding an edge that has a turret on it, which is the thing that makes a carried turret worse than none today.

### 9.4 What this leaves for the human

1. **D-P4-12** — the rail yard's coal, so D-P4-10 (a)'s second half can be built. Recommended: a fourth rubble kind, `coal`, on rail-yard lots at the HQ patch's ~700. Blocks nothing before Gate B.
2. **The hour no longer contains its own end.** `HOUR_END.held` is 3, so §11's enclosure — the HQ's white border when north holds — and the Electricians walking out of civic north are beats a 3,600 s run never reaches. Phase 5's opening should say whether that is the shape of hour one or a hole in it.
3. **The first shade still never happens** in the bot's hour (six runs, unchanged and already reported): §11 and E3 put it at minute 32–47. It is the only finding E-hour still prints.

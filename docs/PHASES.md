...
The fourteen phases and their definitions of done, moved out of `CONSTITUTION.md` by the guardrails task on 2026-09-04 so that the constitution holds only the rules. The rules are `CONSTITUTION.md`; the current state is `PROGRAMME_STATE.md`; the ordered next actions are `ROADMAP.md` §0. Phases 0–3 are history and are not edited; Phase 4's milestone lines say what was built (`SLICE_REPORT.md`); the rest are the plan as written, with the stale sentences the standards audit reported corrected in place.
...
§12–§14 in full: resources (stone, steel, copper, coal, and crude — only once the Refinery is found, which is the Pumpjack's unlock in §13, with the Refinery its only consumer per §12), intermediates, every recipe in a generated `recipes.ts` that the doc's table is built from; the §13 machine list with footprints and nothing outside it — [open: D-P5-2]; belts (one lane, or §14's two — [open: D-P5-3]), undergrounds, splitters, one inserter (§13, D-INSERTERS-1 decided 2026-09-04; the filter it carries is [open: D-P5-5]), chests, the Depot (hand placement only, never a logistics network), trams (one per track, two stops, up to six inserters a stop (§13), lines drawn on the map view — [open: D-P5-6]); rubble as finite typed ore with visible depletion; power complete with proportional brownout (D-B3-4) and its read on the map view — [open: D-P5-1]; slots as real lot geometry, with the doc stating whether an interior lot can hold more than one machine.

Experiments (their names are [open: D-P5-7] — `E10` already names the retired bloom-cadence run cited in §25, C8, D-P3-5 and D-P3-11): **E10 chain throughput** (every chain at its §12 rate), **E11 coal depletion** (when the Turbine hall is needed), **E12 tram sufficiency** (one tram carries the outskirts' steel at 25 h).

**DoD:** the 5 h §18 territory is buildable by a bot from a blueprint file at doc rates — [open: D-P5-4]; a human builds a two-assembler ammo line in the world view in under ten minutes unaided. **STANDARDS rows:** 1.3 / 1.4 belt drag with corners and the underground span shown; 1.10 undo / redo in the command layer; 1.11 copy / paste with a clipboard; 1.16 the hotbar as references to the recipe table with room for §13; 1.17 one binding table; 8.5 one string table; 2.3 splitter priority on its panel; 2.5 a filtering primitive; 2.6 the machine status enum (running / starved / blocked / no power / no ammo) on the panel; 2.7 rates on the machine panel; 2.9 alt-mode data per entity; 3.1 per-item production and consumption counters readable in game; 2.11 / 7.4 / A.9 trams and the driven truck. **STANDARDS:** the `STANDARDS.md` rows for this phase are checked; no new gap was introduced without a row.

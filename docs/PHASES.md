...
The fourteen phases and their definitions of done, moved out of `CONSTITUTION.md` by the guardrails task on 2026-09-04 so that the constitution holds only the rules. The rules are `CONSTITUTION.md`; the current state is `PROGRAMME_STATE.md`; the task order is `PROGRESS.md` (the old `ROADMAP.md` is archived under `archive/pre-phase-5-cleanup/`). Phases 0–3 are history and are not edited; Phase 4's milestone lines say what was built (`SLICE_REPORT.md`); the rest are the plan as written, with the stale sentences the standards audit reported corrected in place. **2026-09-05, RI-00:** `REVISED_DEVELOPMENT_PLAN.md` (adopted, D-RI-1) interleaves these phases — its RI tasks pull the hybrid, the first project, the Junction Heart and the first new enemy ahead of the rest of Phase 5's factory. Each phase below keeps its scope and exit criteria and gains a "moved forward" line naming the RI task that now owns a piece; `PROGRESS.md` is the order.
...
**Moved forward by the plan (2026-09-05, D-RI-1):** the economy, recipes, rubble, coal, the 75-minute hour and the resource accounting → **RI-01**; the goal line and the essential presentation → **RI-02**; physical claims and field deployment → **RI-03**; the minimal tram route → **RI-05**; the rest of the phase (T13 → T17, then T18) runs as **RI-09** after the representative loop has been played (RI-08). The DoD stands.

## Phase 6 — Threat, complete (4 weeks)

**Entry criterion (added 2026-09-05 by the cleanup; met the same day by RI-00):** the one-page definition it asked for — what an expedition is *for*, what the player carries out, what lighting-up triggers and how often it triggers nothing, how an expedition fails and what the failure costs, and what the player learns from one — is `REVISED_DEVELOPMENT_PLAN.md` §4–§9 (D-RI-1), and D-CU-3's order of operations is provisional (b) on the plan's default. §19's shooting cap is a constraint the definition must respect, not evidence that the expedition is fun; that evidence is a played session (T19). **The hybrid is no longer built here**: D-RI-4 superseded D-GB-1-rider, and RI-03 (physical commissioning and field deployment), RI-05 (projects) and RI-06 (the Junction Heart) build it before this phase; it still moves §5, §10, §11, §18 and §19, section by section as each task lands. The enemies of §7 with their logistics answers — crawlers (ammo throughput), shades (lamp coverage, untargetable off-light, substation disable, stacking), hulks (frontage, through the ring, the Cannon line; display name Breaker from RI-10) — plus the plan's Stalker (RI-04) and Conductor (RI-10), each introduced alone and passing rule 2's scope test (the "no fourth" clause went with D-RI-3). **Moved forward:** emergence points, safe interiors and the Stalker → **RI-04**; the Junction Heart → **RI-06**; the Breaker and Conductor, the turret-damage reconciliation (D-B4-3) and the minimum Cannon / Shell chain → **RI-10**; the later bosses → **RI-12**. Rot at tile level with every §5 rule. The fall at the decided time with retake and machine loss. Cannons and shells as the second line with the interior-siting rule. Wells with the all-neighbours kill (D6) and the Relight surge flag. Cached flow fields.
...
Facilities restored by claim plus a delivery on a belt, unlocking their §8 list; some never craftable. Survivors joining on Held, specialists unlocking by toast, the HQ survivor panel as the entire tech screen. §17 placement bands and the validator rules (nothing behind a well, the Foundry at §11's range). No research menu — any unlock without a building or survivor is a stop-and-report. **Moved forward (2026-09-05):** the project record, the rail-yard restoration and the local supply depot → **RI-05**; the remaining reward catalogue (foundry, riverside generation, transformer, survivors) → **RI-11**.
...
Next Fest with the demo; telemetry read against E20's distributions; a balance pass where players diverge from bots. Release with the slider, the presets, the in-game seed browser, the post-Relight hooks, and the doc in the repo with its changelog. After launch, one rule: **rule 2's scope test** (D-RI-3 replaced the "no fourth system" wording on 2026-09-05). Content is seeds, presets and facilities; a proposed mechanic goes through the scope test and §23's rejected-alternatives test in public. **Phases 10–14 are RI-13's** (`PROGRESS.md`): their exit criteria are its acceptance. **STANDARDS:** a post-launch request is scored against `STANDARDS.md` before it is built; a converged gap gets a row, a diverged one goes under `## Considered`.

...
import {ground,rubbleAt,canPlace,truckWorkRoute,actionResult,conservation,stateHash,makeSave,type Command} from '../../../packages/sim/src/index';

mkdirSync(new URL('paid-funded/',import.meta.url));
for(const seed of (process.argv[2]?.split(',').map(Number)??[3,4,5,8,11,13,80,88,102,842,2313,3610])){
 ...
 if(beforeSalvage<10){
  ...
  for(let k=0;k<60&&(st.engineer.inv.steel??0)<10;k++)d.run(1);
  d.send({type:'factory',action:{type:'mineAt',x:-1,y:-1}});assert.ok(st.engineer.inv.steel>=10);
 }
 ...
 writeFileSync(new URL(`paid-funded/seed-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
 writeFileSync(new URL(`paid-funded/save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});
 console.log(JSON.stringify(record));
  ...
  writeFileSync(new URL(`paid-funded/failure-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
  writeFileSync(new URL(`paid-funded/failure-save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});

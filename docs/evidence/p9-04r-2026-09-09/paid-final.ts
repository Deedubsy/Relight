...
import {ground,placementGeometryProblem,MACHINE_COST,rubbleAt,canPlace,truckWorkRoute,actionResult,conservation,stateHash,makeSave,type Command} from '../../../packages/sim/src/index';

mkdirSync(new URL('paid-final/',import.meta.url));
for(const seed of (process.argv[2]?.split(',').map(Number)??[3,4,5,8,11,13,80,88,102,842,2313,3610])){
 ...
 // The longer seed-842 trunk uses more than the starting carried steel budget.
 // Mine finite home salvage before buying the supply chest; no inventory assistance.
 const needed=MACHINE_COST.chest.steel+2;
 const beforeSalvage=st.engineer.inv.steel??0;
 if(beforeSalvage<needed){
  const tile=Array.from(G.blocks[st.campaign!.homeBlock].tiles).find(t=>{const r=rubbleAt(st,t%G.tw,Math.floor(t/G.tw));return r?.type==='steel'&&!r.persistent;});
  assert.notEqual(tile,undefined);const x=tile!%G.tw,y=Math.floor(tile!/G.tw);d.approach(x,y);
  d.send({type:'factory',action:{type:'mineAt',x,y}});
  for(let k=0;k<60&&(st.engineer.inv.steel??0)<needed;k++)d.run(1);
  d.send({type:'factory',action:{type:'mineAt',x:-1,y:-1}});assert.ok(st.engineer.inv.steel>=needed);
 }
 const extraSalvage=(st.engineer.inv.steel??0)-beforeSalvage;
 ...
 // Leave the narrow street after loading so the engineer does not block the truck.
 d.approach(depot.x,depot.y,depot.size);
 const destination=(()=>{for(let r=11;r<=18;r++)for(let y=Math.floor(t.y)-r;y<=t.y+r;y++)for(let x=Math.floor(t.x)-r;x<=t.x+r;x++){
  if(Math.hypot(x+.5-t.x,y+.5-t.y)<11||placementGeometryProblem(st,'belt',x,y,1)||placementGeometryProblem(st,'belt',x+1,y,1))continue;
 ...
 const record={seed,extraSalvage,assistance,surveyVersion:st.campaign!.expansion!.surveyVersion,seconds:st.t,tracks:network.path.length,stops:network.stops.length,freightDelivered:last.cargo?.steel,truck:{source:[source.x,source.y],destination:[destination.x,destination.y],routeSteps:destination.route.length,start,end:{x:t.x,y:t.y},phases:[...phases],order:order.status,pocketsUnchanged:true},conserved:true,hash,replay,continuation};
 writeFileSync(new URL(`paid-final/seed-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
 writeFileSync(new URL(`paid-final/save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});
 console.log(JSON.stringify(record));
  ...
  writeFileSync(new URL(`paid-final/failure-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
  writeFileSync(new URL(`paid-final/failure-save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});

...
import {ground,MACHINE_COST,rubbleAt,canPlace,truckWorkRoute,actionResult,conservation,stateHash,makeSave,type Command} from '../../../packages/sim/src/index';

mkdirSync(new URL('paid-funded-final/',import.meta.url));
for(const seed of (process.argv[2]?.split(',').map(Number)??[3,4,5,8,11,13,80,88,102,842,2313,3610])){
 ...
 writeFileSync(new URL(`paid-funded-final/seed-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
 writeFileSync(new URL(`paid-funded-final/save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});
 console.log(JSON.stringify(record));
  ...
  writeFileSync(new URL(`paid-funded-final/failure-${seed}.json`,import.meta.url),JSON.stringify(record,null,2)+'\n',{flag:'wx'});
  writeFileSync(new URL(`paid-funded-final/failure-save-${seed}.json`,import.meta.url),JSON.stringify(makeSave(st,{log:d.log,logComplete:true})),{flag:'wx'});

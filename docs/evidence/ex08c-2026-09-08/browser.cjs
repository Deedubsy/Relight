...
const load=async(name)=>{await page.goto('http://127.0.0.1:5179/?rules=exploration-v2&seed=3&view=world&state=/'+name+'.json');await page.waitForFunction(()=>window.__relight?.world);await page.waitForTimeout(400);};
try{
 for(const width of [1366,900])for(const name of ['fresh','network','turbine']){

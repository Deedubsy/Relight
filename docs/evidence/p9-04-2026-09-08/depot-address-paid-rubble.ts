...
for(const seed of (process.argv[2]?.split(',').map(Number)??[102])){
 ...
 for(const p of [...path.map(tile=>({x:tile%G.tw,y:Math.floor(tile/G.tw),size:1,kind:'track' as const})),...[...e.stops,district.stop].map(([x,y])=>({x,y,size:2,kind:'tramstop' as const}))]){
  ...
  const storedTrack=Math.min(20,st.engineer.inv.track??0);assert.equal(d.put('track',storedTrack,depot),storedTrack);
  let mined=0;
  for(let y=p.y;y<p.y+p.size;y++)for(let x=p.x;x<p.x+p.size;x++){
   for(let attempt=0;rubbleAt(st,x,y)&&attempt<20;attempt++){
    if(invStacks(st.engineer.inv)>30)for(const item of ['steel','copper','coal','stone']){const n=(st.engineer.inv[item]??0)-(item==='stone'?0:100);if(n>0)d.put(item,n,depot);}
  ...
  const stone=st.engineer.inv.stone??0;if(stone)assert.equal(d.put('stone',stone,depot),stone);assert.equal(d.take('track',storedTrack,depot),storedTrack);

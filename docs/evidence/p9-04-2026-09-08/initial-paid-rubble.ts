...
mkdirSync(new URL('paid-rubble/',import.meta.url));
 ...
 for(const p of [...path.map(tile=>({x:tile%G.tw,y:Math.floor(tile/G.tw),size:1,kind:'track' as const})),...[...e.stops,district.stop].map(([x,y])=>({x,y,size:2,kind:'tramstop' as const}))]){
  ...
  for(let y=p.y;y<p.y+p.size;y++)for(let x=p.x;x<p.x+p.size;x++){
   for(let attempt=0;rubbleAt(st,x,y)&&attempt<20;attempt++){
    if(invStacks(st.engineer.inv)>30)for(const item of ['steel','copper','coal']){const n=(st.engineer.inv[item]??0)-100;if(n>0)d.put(item,n,depot);}
    ...
    const remaining=rubbleAt(st,x,y)?.units??0;assert.ok(remaining<units,'ordinary mining must progress');mined+=units-remaining;

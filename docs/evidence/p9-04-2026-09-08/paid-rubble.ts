...
import {ground,blockOfTile,campaignThrottle,canPlace,rubbleAt,invStacks,truckWorkRoute,actionResult,conservation,stateHash,makeSave,type Command} from '../../../packages/sim/src/index';
...
for(const seed of (process.argv[2]?.split(',').map(Number)??[102])){
 ...
 const record={seed,assistance,clearedFootprints,freightPass:last.cargo?.steel===5,power:{homeBlock:st.campaign!.homeBlock,stops:stops.map(m=>({id:m.id,x:m.x,y:m.y,block:blockOfTile(st,m.x,m.y),throttle:campaignThrottle(st,blockOfTile(st,m.x,m.y))})),generators:st.flow!.machines.filter(m=>m.kind==='generator').map(m=>({id:m.id,x:m.x,y:m.y,block:blockOfTile(st,m.x,m.y),coal:m.inv.coal??0}))},surveyVersion:st.campaign!.expansion!.surveyVersion,seconds:st.t,tracks:network.path.length,stops:network.stops.length,freightDelivered:last.cargo?.steel??0,truck:{source:[source.x,source.y],destination:[destination.x,destination.y],routeSteps:destination.route.length,start,end:{x:t.x,y:t.y},phases:[...phases],order:order.status,pocketsUnchanged:true},conserved:true,hash,replay,continuation};

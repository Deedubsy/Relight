...
for(const seed of (process.argv[2]?.split(',').map(Number)??[102])){
 ...
 const withdraw=(item:string,n:number)=>{d.approach(depot.x,depot.y,depot.size);return d.take(item,n);};
 ...
 for(const p of [...path.map(tile=>({x:tile%G.tw,y:Math.floor(tile/G.tw),size:1,kind:'track' as const})),...[...e.stops,district.stop].map(([x,y])=>({x,y,size:2,kind:'tramstop' as const}))]){
  ...
  const storedTrack=Math.min(20,st.engineer.inv.track??0);assert.equal(deposit('track',storedTrack),storedTrack);
  let mined=0;
  ...
  const stone=st.engineer.inv.stone??0;if(stone)assert.equal(deposit('stone',stone),stone);assert.equal(withdraw('track',storedTrack),storedTrack);

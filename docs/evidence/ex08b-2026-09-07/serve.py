...
with tempfile.TemporaryDirectory(prefix='relight-ex08b-') as temp:
    ...
    with http.server.ThreadingHTTPServer(('127.0.0.1', 5177), handler) as server:
        print(manifest['url'], flush=True)
        print('Frozen EX-08H-B build. Fresh and pre-warning starts stay paused until the player starts. Ctrl+C stops the server.', flush=True)
        try:

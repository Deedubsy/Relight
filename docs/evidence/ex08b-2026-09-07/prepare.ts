...
const out='docs/evidence/ex08b-2026-09-07',ref={kind:'campaign',ruleset:'exploration-v2',opening:'culdesac-v1'} as const,provenance=stamp(ref);
const evidence=JSON.parse(readFileSync('docs/evidence/p6-03-2026-09-07/recorded/campaign/E-defence-turrets-seed3.json','utf8'));
...
const starts=[];
for(const [name,st,commands,description] of [['fresh',initial,[],'Untouched ordinary tick-zero campaign; no assistance.'],['network',network,log,'Scripted opening replayed from ordinary starting stock and full paid command history; player did not perform the opening. No resource/position/clock/HP injection. Not an opening-time or unaided discovery observation.']] as const){
...
const settings={...provenance,session:'EX-08H-B / P6-H',human_play:'not_run',starts,defaultStart:'network',speed:'1x; pauses/interventions recorded',difficulty:'unchanged default campaign rules',config:initial.config,clock:CAMPAIGN_RULES,threat:CAMPAIGN_THREAT,defence:DEFENCE,districts:DISTRICT_ECONOMY,discovery:DISCOVERY};
...
console.log('PASS: fresh start and ordinary-command network prefix verified, conserved, load paused and continue deterministically.');

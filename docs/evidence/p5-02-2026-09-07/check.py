"""Verify P5-02 references, handoff, tested source/build hashes and preserved EX-08A archives."""
...
if '--record' in sys.argv:
    ...
    data = {
        ...
        'status': 'P5-02 uncommitted working-tree verification; parent commit is not the tested source identity',
...
names = ['CLAUDE.md', 'README.md'] + ['docs/' + n + '.md' for n in ['DECISIONS', 'PROGRESS', 'PROGRAMME_STATE', 'PHASES', 'RELIGHT-design', 'EXPLORATION_DEFENCE_PLAN', 'PHASE_5_SCOPE_REPORT', 'P5_02_ROUTING_REPORT', 'STANDARDS']]
count = 0
for name in names:
    p = root / name
...
assert tasks['P5-02'][3] == tasks['T14'][3] == 'done'
assert next(r[0] for r in rows if r[3] in ['todo', 'in_progress']) == 'P5-03'
for i in range(4, 6):
...
assert len(ids) == len(set(ids)) and 'D-EX-23' in ids
assert re.search(r'^\| D-EX-Q08 \|.*\| decided \|', decisions, re.M)
assert re.search(r'^\| D-EX-Q07 \|.*\| open \|', decisions, re.M)
assert '# pass 214' in (out / 'suite-complete.log').read_text(encoding='utf-8')
...
print('P5-02/T14 technical work complete; P5-03 next; human gates blocked; Q08 decided/Q07 open; 214 tests and browser replay confirmed.')

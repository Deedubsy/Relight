"""Verify P5-01 references, handoff, tested source/build hashes and preserved EX-08A archives."""
...
if '--record' in sys.argv:
    ...
    data = {
        ...
        'status': 'P5-01 uncommitted working-tree verification; parent commit is not the tested source identity',
        ...
        'verification_files': {p.name: digest(p) for p in sorted(out.iterdir()) if p.suffix in ['.log', '.json', '.png', '.cjs', '.sh'] and p.name not in ['build-manifest.json', 'consistency.log']},
    }
...
names = ['CLAUDE.md', 'README.md'] + ['docs/' + n + '.md' for n in ['DECISIONS', 'PROGRESS', 'PROGRAMME_STATE', 'PHASES', 'RELIGHT-design', 'EXPLORATION_DEFENCE_PLAN', 'PHASE_5_SCOPE_REPORT', 'P5_01_CONSTRUCTION_REPORT', 'STANDARDS']]
count = 0
for name in names:
    p = root / name
...
assert tasks['P5-01'][3] == tasks['T13'][3] == 'done'
assert next(r[0] for r in rows if r[3] in ['todo', 'in_progress']) == 'P5-02'
for i in range(3, 6):
...
assert len(ids) == len(set(ids)) and 'D-EX-22' in ids
assert re.search(r'^\| D-EX-Q08 \|.*\| decided \|', decisions, re.M)
assert re.search(r'^\| D-EX-Q07 \|.*\| open \|', decisions, re.M)
assert '# pass 203' in (out / 'suite-verified.log').read_text(encoding='utf-8')
...
print('P5-01/T13 technical work complete; P5-02 next; human gates blocked; Q08 decided/Q07 open; 203 tests and browser replay confirmed.')

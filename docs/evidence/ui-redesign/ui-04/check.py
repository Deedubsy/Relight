...
m=json.loads(read(out/'manifest.json'));prior=root/'docs/evidence/ui-redesign/ui-03';old=json.loads(read(prior/'manifest.json'))
assert len(m['source_files'])==214 and len(m['changes'])==13 and len(m['build_files'])==9
...
for n,h in v['evidence'].items():assert sha(prior/n)==h,('UI-03',n)
...
for name,count in [('test.log',370),('focused-final.log',35)]:
...
b=json.loads(read(out/'browser-result.json'));assert len(b['rows'])==6 and not b['errors']
for r in b['rows']:
 if r['case']=='identity':
  for k in ['paidAssembler','stalled','recipe','pin','removedId','replacementNotRetargeted','filter','sourceReadOnly']:assert r[k],k
  assert r['replay']['same']
 elif r['case']=='network':
  for k in ['working','remoteRefusal','stockConserved','stationRequests']:assert r[k],k
 else:
  for k in ['chestTransfer','truckScopes','constructionDelivery']:assert r[k],k
...
for n in ['stalled-1366.png','stalled-900.png','working-1366.png','working-900.png','home-900.png','truck-900.png']:assert (out/n).stat().st_size>1000
rows={line.split('|')[1].strip():line.split('|') for line in read(root/'docs/PROGRESS.md').splitlines() if line.startswith('| ')}
for n in ['RI-02B-UI-01','RI-02B-UI-02','RI-02B-UI-03','RI-02B-UI-04']:assert rows[n][4].strip()=='done'
assert rows['RI-02B-UI-05'][4].strip()=='todo'
for n in ['RI-02B-UI-06','RI-02B-UI-07']:assert rows[n][4].strip()=='blocked'
...
result={'status':'UI-04 implementation and automated checks complete; human play not_run','sourceInputs':214,'changedFiles':m['changes'],'productionFiles':6,'preparedFixtures':3,'fullTests':370,'focusedTests':35,'browserWidths':[1366,900],'browserCases':6,'shellRegressionWidths':[1366,900],'normalTextMinimumContrast':min(x['ratio'] for x in r['contrast']['text']),'controlBorderMinimumContrast':min(x['ratio'] for x in r['contrast']['borders']),'localReferences':links,'priorEvidence':'UI-03, P9-05, P9-04R and EX-08D preserved','documents':{n:sha(root/n) for n in names},'evidence':{p.relative_to(out).as_posix():sha(p) for p in out.rglob('*') if p.is_file() and p.name not in ['verification.json','consistency.log']}}
(out/'verification.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8',newline='\n')
print(f'PASS: 214 sources / 13 changes / 9 archived HTTP files; 370 full + 35 focused; types/build/lint/docs; 6 browser cases and both-width shell/save/replay/contrast; {links} references; frozen evidence/human gates preserved; UI-05 next.')

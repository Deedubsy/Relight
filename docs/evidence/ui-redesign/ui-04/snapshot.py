...
root=Path(__file__).resolve().parents[4];out=Path(__file__).resolve().parent;prior=root/'docs/evidence/ui-redesign/ui-03'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
old=json.loads((prior/'manifest.json').read_text());files=sorted(set(old['source_files'])|{'packages/game/src/inspectionPanel.ts','packages/game/src/inventoryPanel.ts'})
sources={n:sha(root/n) for n in files};changes=[n for n,h in sources.items() if old['source_files'].get(n)!=h]
assert all(n.startswith('packages/game/') or n in {'packages/sim/src/inspection.ts','packages/sim/src/goal.ts','packages/sim/src/flow.ts','packages/sim/src/truck.ts','packages/sim/test/inspection.test.ts'} for n in changes)
...
with zipfile.ZipFile(prior/'source.zip') as z:
 for n in changes:
  ...
  patch.extend(difflib.unified_diff(before,(root/n).read_text(encoding='utf-8').splitlines(True),fromfile='UI-03/'+n,tofile='UI-04/'+n))

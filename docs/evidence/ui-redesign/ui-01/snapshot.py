...
root=Path(__file__).resolve().parents[4];out=Path(__file__).resolve().parent;prior=root/'docs/evidence/p9-05-2026-09-09'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
old=json.loads((prior/'build-manifest.json').read_text());files=sorted(set(old['source_files'])|{'packages/game/src/uiShell.ts'})
sources={n:sha(root/n) for n in files};changes=[n for n,h in sources.items() if old['source_files'].get(n)!=h]
assert all(n.startswith('packages/game/') for n in changes)
dist=root/'packages/game/dist'
for name in ['fresh','construction','network']:shutil.copyfile(prior/'campaign'/(name+'.json'),dist/('ui-'+name+'.json'))
...
with zipfile.ZipFile(prior/'source.zip') as z:
 for n in changes:
  ...
  patch.extend(difflib.unified_diff(before,(root/n).read_text(encoding='utf-8').splitlines(True),fromfile='P9-05/'+n,tofile='UI-01/'+n))
(out/'source-delta.patch').write_text(''.join(patch),encoding='utf-8',newline='\n')
(out/'manifest.json').write_text(json.dumps(m,indent=2)+'\n',encoding='utf-8',newline='\n')
print('Archived',len(sources),'source inputs;',len(build),'served production/fixture files;',len(changes),'UI file changes; simulation and harness unchanged.')

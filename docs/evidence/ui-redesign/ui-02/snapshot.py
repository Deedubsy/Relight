...
root=Path(__file__).resolve().parents[4];out=Path(__file__).resolve().parent;prior=root/'docs/evidence/ui-redesign/ui-01'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
old=json.loads((prior/'manifest.json').read_text());files=sorted(set(old['source_files'])|{'packages/game/src/hud.ts','packages/sim/src/interaction.ts','packages/sim/test/uiGuidance.test.ts'})
sources={n:sha(root/n) for n in files};changes=[n for n,h in sources.items() if old['source_files'].get(n)!=h]
assert all(n.startswith('packages/game/') or n in {'packages/sim/src/goal.ts','packages/sim/src/flow.ts','packages/sim/src/interaction.ts','packages/sim/src/index.ts','packages/sim/test/campaign.test.ts','packages/sim/test/uiGuidance.test.ts'} for n in changes)
...
with zipfile.ZipFile(prior/'source.zip') as z:
 for n in changes:
  ...
  patch.extend(difflib.unified_diff(before,(root/n).read_text(encoding='utf-8').splitlines(True),fromfile='UI-01/'+n,tofile='UI-02/'+n))

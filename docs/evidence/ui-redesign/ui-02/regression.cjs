...
const key=async(k)=>{await page.locator('#map > canvas').focus();await page.keyboard.press(k);};
try{
 for(const [width,height] of [[1366,768],[900,768]]){
  ...
  await page.screenshot({path:path.join(__dirname,'regression-'+`pause-${width}.png`)});
  ...
  await name.focus();await page.keyboard.press('Escape');await page.locator('#map > canvas').focus();await page.waitForTimeout(300);assert.deepEqual((await read()).e,stopped.e);await page.keyboard.up('w');await key('p');assert.equal((await read()).speed,0);
  ...
  await page.screenshot({path:path.join(__dirname,'regression-'+`build-${width}.png`)});await page.getByRole('button',{name:'Close panel'}).click();
 ...
 await page.setViewportSize({width:1280,height:720});await load('fresh');await page.waitForTimeout(5200);await page.screenshot({path:path.join(__dirname,'regression-'+'opening-1280.png')});
 ...
 assert.deepEqual(errors,[]);fs.writeFileSync(path.join(__dirname,'regression-'+'browser-result.json'),JSON.stringify({browser:browser.version(),rows,contrast,errors,method:'Actual production entry with frozen P9-05 ordinary-command prepared starts. UI actions and real keyboard/pointer input; source queries only select legal drag cells. No gameplay stock/position injection. Not human usability acceptance.'},null,2)+'\n');
}catch(e){await page.screenshot({path:path.join(__dirname,'regression-'+'browser-failure.png')});console.error(await page.locator('body').innerText());throw e;}finally{await browser.close();}

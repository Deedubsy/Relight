...
root=Path(__file__).resolve().parents[4];out=Path(__file__).resolve().parent;prior=out.parent/'ui-06'
...
with zipfile.ZipFile(prior/'source.zip') as z:
 for n in changes:
  ...
  patch.extend(difflib.unified_diff(before,(root/n).read_text(encoding='utf-8').splitlines(True),fromfile='UI-06/'+n,tofile='UI-07/'+n))

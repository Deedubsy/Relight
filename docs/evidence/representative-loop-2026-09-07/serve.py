...
with tempfile.TemporaryDirectory(prefix='relight-ex08a-') as temp:
    ...
    with http.server.ThreadingHTTPServer(('127.0.0.1', 5175), handler) as server:
        print(manifest['url'], flush=True)
        print('Frozen EX-08H-A build. Tick-zero start stays paused until the player starts. Ctrl+C stops the server.', flush=True)
        try:

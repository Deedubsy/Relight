"""Capture/verify P6-02 source, build, checks and preserved historical artifacts."""
...
latest=json.loads(read(root/'docs/evidence/p6-01-2026-09-07/build-manifest.json'))
...
for n,h in latest['verification_files'].items(): assert sha(root/'docs/evidence/p6-01-2026-09-07'/n)==h,n
for n,h in latest['archives'].items(): assert sha(root/'docs/evidence/p6-01-2026-09-07'/n)==h,n
for folder in ['p6-01-2026-09-07','phase6-review-2026-09-07']:
    for p in (root/'docs/evidence'/folder).iterdir():
        if p.is_file(): historical[p.relative_to(root).as_posix()]=sha(p)
historical['docs/P6_01_RELIABILITY_REPORT.md']=sha(root/'docs/P6_01_RELIABILITY_REPORT.md')
print(f'Preserved {len(historical)} historical report/evidence files, including verified P6-01 logs/archives.')

changed={n for n,h in latest['source_files'].items() if sha(root/n)!=h}
expected={'packages/game/src/controls.ts','packages/game/src/main.ts','packages/game/src/panel.ts',
          'packages/game/src/style.css','packages/game/src/view.ts','packages/game/src/worldScene.ts',
          'packages/sim/src/campaignThreat.ts','packages/sim/src/threat.ts'}
assert changed==expected,changed
sources={n:sha(root/n) for n in sorted(set(latest['source_files'])|{'packages/sim/test/campaignInformation.test.ts'})}
...
assert passed==247,passed
assert 'built in' in read(out/'typecheck-browser.log')
...
for name in ['browser-result.json','arrival-result.json','interaction-result.json']:
    result=json.loads(read(out/name));assert result['errors']==[],name
result=json.loads(read(out/'interaction-result.json'))
assert all(r['endSim']>r['startSim']+3 and r['successful']==r['edits'] and r['lightPaints']>0 for r in result['rows'])
docs=['CLAUDE.md','README.md']+['docs/'+n+'.md' for n in ['PROGRESS','PROGRAMME_STATE','DECISIONS',
 'RELIGHT-design','PHASES','STANDARDS','EXPLORATION_DEFENCE_PLAN','P6_02_INFORMATION_REPORT']]
links=0
pending=[]
for name in docs:
    p=root/name
...
for line in read(root/'docs/PROGRESS.md').splitlines():
    ...
    rows[cells[0]]=cells
assert rows['P6-02'][3]=='done' and rows['P6-03'][3]=='todo'
assert next(n for n,r in rows.items() if r[3] in ['todo','in_progress'])=='P6-03'
assert all(rows[n][3]=='blocked' for n in ['P6-04','P6-H','EX-08B','EX-08H','T18'])
print(f'{passed} tests pass; final build, lint, legacy snapshot, docs/freshness and {links} local links checked; P6-03 next.')
...
if '--capture' in sys.argv:
    ...
    logs={p.name:sha(p) for p in out.iterdir() if p.suffix in ['.log','.py','.sh','.cjs','.ts','.json','.png'] and p.name!='build-manifest.json' and p.name!='consistency.log'}
    data={'recorded_at':datetime.now(timezone.utc).isoformat(),
          ...
          'status':'P6-02 uncommitted source/build; owner gameplay remains the earlier Phase 5 result',
...
print(f'Verified {len(sources)} source/test/build inputs and {len(builds)} production files in independent P6-02 archives.')

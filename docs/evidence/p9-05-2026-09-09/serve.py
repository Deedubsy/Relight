...
with tempfile.TemporaryDirectory(prefix='relight-p905-') as temp:
    ...
    with http.server.ThreadingHTTPServer(('127.0.0.1', 5181), handler) as server:
        print(manifest['url'], flush=True)
        print('Frozen post-P9 build. Fresh, pre-warning and Turbine starts stay paused until the player starts. Ctrl+C stops the server.', flush=True)
        try:

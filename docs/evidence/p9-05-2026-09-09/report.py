...
import json
p=Path('docs/evidence/p9-05-2026-09-09');m=json.loads((p/'build-manifest.json').read_text());b=json.loads((p/'browser-result.json').read_text());s=json.loads((p/'settings.json').read_text())
report='''# P9-05 — city engineering readiness and human preparation
...
|---|---|---|---|---|---|
'''
for r in b['rows']:report+=f"| {r['name']} / {r['width']} | {r['successful']}/{r['edits']} | {r['meanMs']:.2f} | {r['p95Ms']:.2f} | {r['maxMs']:.2f} | {r['lightPaintMeanMs']:.2f} |\n"
report+='''
...
'''
Path('docs/P9_05_READINESS_REPORT.md').write_text(report,encoding='utf-8',newline='\n')

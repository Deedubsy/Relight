...
try{
 assert.deepEqual(st.engineer.inv,{});
 mine('steel',340);mine('copper',160);mine('coal',100);milestones.push({step:'ordinary hand-mined bootstrap',t:st.t});
 const generator=build('generator');checked({type:'factory',action:{type:'feed',x:generator.x,y:generator.y}});
 const assembler=build('assembler'),turret=build('turret');
 const [wx,wy]=workbenchTile(st);walk(wx,wy);checked({type:'equipment',action:{type:'craftRifle'}});run(6.1);checked({type:'equipment',action:{type:'equip',slot:0,item:'rifle:1'}});
 checked({type:'factory',action:{type:'craft',item:'magazine',count:8}});run(49);
 near(turret.x,turret.y);checked({type:'factory',action:{type:'feed',x:turret.x,y:turret.y}});
 walk(wx,wy);checked({type:'factory',action:{type:'craft',item:'magazine',count:52}});run(313);reload();
 near(assembler.x,assembler.y);checked({type:'factory',action:{type:'machineTransfer',id:assembler.id,item:'steel',n:40,put:true}});checked({type:'factory',action:{type:'machineTransfer',id:assembler.id,item:'copper',n:20,put:true}});
 for(let x=turret.x+2;x<assembler.x;x++){near(x,assembler.y+1);checked({type:'construct',edits:[{action:'place',item:'belt',x,y:assembler.y+1,dir:3}]});}
 run(12);assert.ok(assembler.out>0||assembler.observation?.produced.magazine,'Home Assembler made ammunition');
 milestones.push({step:'paid Rifle + 600 ordinary rounds; turret loaded',t:st.t,inv:{...st.engineer.inv}});console.log(milestones.at(-1));
 for(const camp of FIRST_CAMPS){near(camp.x,camp.y+14);fight(camp.id);near(camp.x,camp.y);checked({type:'region',action:{type:'claimKey',id:camp.id}});run(16);}
 near(66,62);checked({type:'region',action:{type:'openFreight',id:'freight'}});fight('freight');
 const core=st.campaign!.progression!.sites.find(s=>s.item==='core1')!;near(core.x,core.y);checked({type:'progression',action:{type:'recover',id:core.id}});
 const plant=st.campaign!.progression!.sites.find(s=>s.kind==='plant')!;near(plant.x,plant.y);checked({type:'progression',action:{type:'deliver',id:plant.id}});checked({type:'progression',action:{type:'activate',id:plant.id}});
 milestones.push({step:'chosen first plant commissioned from ordinary resources',t:st.t,hp:st.engineer.hp,rounds:st.engineer.fired});
 assert.equal(stateProblem(st),'');assert.ok(conservation(st).ok,conservation(st).problems.join(';'));
 assert.ok(st.campaign!.defence!.bases.find(b=>b.block===st.campaign!.homeBlock)!.hp>0,'supplied Home defence survived harassment');
 writeFileSync('docs/evidence/gp-checkpoint/ordinary-save.json',JSON.stringify(makeSave(st,{log,logComplete:true})));
 samples.sort((a,b)=>a-b);writeFileSync('docs/evidence/gp-checkpoint/ordinary-result.json',JSON.stringify({label:'Headless scripted ordinary-resource command route; simulation 1x, accelerated execution; not human pacing',milestones,tickMs:{median:samples[Math.floor(samples.length*.5)],p95:samples[Math.floor(samples.length*.95)]},ledger:conservation(st),commands:log.length},null,2));console.log(milestones);
}catch(error){writeFileSync('docs/evidence/gp-checkpoint/ordinary-failure.json',JSON.stringify({error:String(error),milestones,t:st.t,inv:st.engineer.inv,position:[st.engineer.x,st.engineer.y],state:st}));throw error;}

...
prior=json.loads((root/'docs/evidence/p8-03-2026-09-08/manifest.json').read_text(encoding='utf-8-sig'))
files=sorted(set(prior['source_files'])|{'packages/sim/test/areaRemoval.test.ts'})
build=root/'packages/game/dist';production=sorted(str(p.relative_to(build)).replace('\\','/') for p in build.rglob('*') if p.is_file())
manifest={'task':'P8-04','parent_commit':prior['parent_commit'],'campaign_metadata':10,'recruits_version':4,'plan_schema':1,'truck_work_schema':1,'source_files':{n:sha(root/n) for n in files},'build_files':{n:sha(build/n) for n in production},'archives':{}}

...
for file,total in [('full-test.log',313),('focused-final.log',40)]:
...
comparison=json.loads(read(out/'comparison.json'))['rows'];assert [r['commands'] for r in comparison]==[24,10]
assert comparison[0]['final']==comparison[1]['final'] and all(r['failures']==0 for r in comparison)
docs=['CLAUDE.md','README.md']+['docs/'+n+'.md' for n in ['PROGRESS','PROGRAMME_STATE','DECISIONS','RELIGHT-design','EXPLORATION_DEFENCE_PLAN','PHASES','STANDARDS','CAMPAIGN_RULES','P8_04_REMOVAL_REPORT']]
links=0
for n in docs:
 p=root/n
...
assert rows['P8-04'][4].strip()=='done' and rows['P8-05'][4].strip()=='todo'
for n in ['EX-08H','P6-H','P7-H','P8-H','T18']:assert rows[n][4].strip()=='blocked' and 'EX-08D' in rows[n][5],n
result={'sourceInputs':len(m['source_files']),'productionFiles':len(m['build_files']),'documents':{n:sha(root/n) for n in docs},'evidence':{p.relative_to(out).as_posix():sha(p) for p in out.rglob('*') if p.is_file() and p.name not in ['verification.json','consistency.log']},'localReferences':links,'priorCampaignMeasurements':'unchanged (P8-03)', 'retainedP803':'unchanged','retainedP802':'unchanged','retainedP801':'unchanged','retainedEX08C':'unchanged','retainedP800':'unchanged','retainedP705':'unchanged','retainedEX08B':'unchanged','human_play':'not_run'}
(out/'verification.json').write_bytes(json.dumps(result,indent=2).encode('utf-8'))
print(f'PASS: {len(m["source_files"])} source inputs / {len(m["build_files"])} production files and archives; 313 broad + 40 final focused tests; both browser widths and full replay; {links} local references; P8-05 next and human tasks preserved; prior evidence unchanged.')

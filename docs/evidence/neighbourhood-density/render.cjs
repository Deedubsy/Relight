 ...
 const result=await page.evaluate(async({phaserURL,data})=>{const{default:Phaser}=await import(phaserURL);return new Promise((resolve,reject)=>{
  const failures=[];window.reviewGame=new Phaser.Game({type:Phaser.AUTO,width:1440,height:900,audio:{noAudio:true},banner:false,scene:{
   ...
   create(){window.reviewScene=this;for(const o of data.displayList.filter(o=>o.type==='Image'))this.add.image(o.x??0,o.y??0,o.texture.key).setName(o.label).setOrigin(o.originX??.5,o.originY??.5).setScale(o.scaleX??1,o.scaleY??1);
    this.cameras.main.setZoom(.24).centerOn(99*32,378*32);const ref=this.textures.get('editor-riverfront-ground').getSourceImage();resolve({objects:this.children.list.length,reference:[ref.width,ref.height],failures});}
  }});setTimeout(()=>reject(Error('Scene load timeout')),20000);
 });},{phaserURL,data});
 if(result.objects!==302||result.failures.length||errors.length)throw Error(JSON.stringify({result,errors}));
 await page.screenshot({path:'docs/evidence/neighbourhood-density/scene-neighbourhood.png'});
 await page.evaluate(()=>reviewScene.cameras.main.setZoom(.048).centerOn(864*16,576*16));
 await page.screenshot({path:'docs/evidence/neighbourhood-density/scene-city.png'});
 fs.writeFileSync('docs/evidence/neighbourhood-density/render.json',JSON.stringify({nativeEditorUI:false,renderer:'Phaser using the native scene object transforms',...result,errors},null,2));console.log(JSON.stringify(result));

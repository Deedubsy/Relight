...
for file,total in [('full-test.log',303),('focused-final.log',30),('provenance-tests.log',4)]:
...
for r in browser['rows']:
 assert r['replay']['same'] and r['reloaded'] and not r['overflow']
 assert r['paused']['truck']['work']['phase']=='paused' and r['final']['orders'][0]['status']=='completed' and r['final']['truck']['work']['phase']=='complete'
 assert r['paused']['pockets']==r['final']['pockets'] and r['paused']['engineer']==r['final']['engineer']
 assert 'travelling' in r['observed']['phases']
...
for n in ['docs-tool-typecheck.log','harness-final-typecheck.log']:assert (out/n).exists() and not read(out/n).strip()
assert sha(out/'truck-ready.json')==sha(root/'packages/game/dist/truck-ready.json')
probe=json.loads(read(out/'route-probe.json'));assert not probe['queryMutatedState'] and probe['reachableSteps']>0
old=json.loads(read(out/'previous-campaign.json'));assert sha(out/'previous-campaign.zip')==old['archive_sha256']
with zipfile.ZipFile(out/'previous-campaign.zip') as z:
 for n,h in old['files'].items():assert hashlib.sha256(z.read(n)).hexdigest()==h,n
runs=sorted((out/'campaign').glob('*.json'));assert len(runs)==12
campaignChecks=0
for p in runs:
 data=json.loads(read(p));assert all(c['pass'] for c in data['checks']),p;campaignChecks+=len(data['checks'])
 assert sha(root/'docs/experiments/campaign'/p.name)==sha(p)
 for n,h in data['source_files'].items():assert sha(root/n)==h,('campaign source',n)
 if data['id']=='E-logistics':assert data['measurements']['hours']==5
docs=['CLAUDE.md','README.md']+['docs/'+n+'.md' for n in ['PROGRESS','PROGRAMME_STATE','DECISIONS','RELIGHT-design','EXPLORATION_DEFENCE_PLAN','PHASES','STANDARDS','CAMPAIGN_RULES','P8_03_TRUCK_REPORT']]
links=0
for n in docs:
 p=root/n
...
assert rows['P8-03'][4].strip()=='done' and rows['P8-04'][4].strip()=='todo'
for n in ['EX-08H','P6-H','P7-H','P8-H','T18']:assert rows[n][4].strip()=='blocked' and 'EX-08D' in rows[n][5],n
result={'sourceInputs':len(m['source_files']),'productionFiles':len(m['build_files']),'documents':{n:sha(root/n) for n in docs},'evidence':{p.relative_to(out).as_posix():sha(p) for p in out.rglob('*') if p.is_file() and p.name not in ['verification.json','consistency.log']},'localReferences':links,'campaignChecks':campaignChecks,'retainedP802':'unchanged','retainedP801':'unchanged','retainedEX08C':'unchanged','retainedP800':'unchanged','retainedP705':'unchanged','retainedEX08B':'unchanged','human_play':'not_run'}
(out/'verification.json').write_bytes(json.dumps(result,indent=2).encode('utf-8'))
print(f'PASS: {len(m["source_files"])} source inputs / {len(m["build_files"])} production files and archives; 303 broad + 30 focused + 4 provenance tests; both browser widths and saved job/full replay; 12 campaign runs/{campaignChecks} checks; {links} local references; P8-04 next and human tasks preserved; earlier evidence unchanged.')

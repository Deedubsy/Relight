...
- ~~**E-rifle at tile scale** (the block-scale E-rifle never sees the rifle decide a rescue: twelve of twelve hold either way) → before Gate B if a go falls free; otherwise Absorb Gate B, with M4's bodies.~~ **Closed 2026-09-05** by `PROGRESS.md` T4: at tile scale the rifle decides three rescues outright and delays three more (`EXPERIMENTS.md` `E-rifle-tile-rescue`). See the re-read at the foot of this file.
...
## Re-read at the layout and readability pass (2026-09-05, `LAYOUT_PASS_REPORT.md` — drawing only, cheap checks green)

- **Kept, with a phase**: the four STANDARDS rows the pass exists to enable (**4.3** the key strip on the HUD, **B.3** / **B.6** the kerb pip's segment state and empty turret, **C.2** the Depot from the far edge) → **a human minute each**, and the walkthrough that carries them (T5) was waived, not run, so `ROADMAP.md` §6 still reads `closed: 0 of 46`; the **stranger test** (§2's own success condition — street, lot edge, lit area, rubble, Depot, engineer, pointed at unaided) → **a human**, with `docs/layout-pass/STRANGER_TEST.md` still unwritten; the **art-direction lean** (grim-and-quiet vs saturated, `ROADMAP.md` §7 expected the readability pass to ask) → **Phase 12**, unasked: the pass is flat colour on the palette already in the code, as §2 requires.
- **Parked here from this pass**: the **light-map blur's frame cost** (7–12 ms of arithmetic per repaint in Node, eight repaints a second, no browser number) → **the next verification pass**, as a re-run of the 900 s soak's frame table with the burn-off running; if the five existing bursts over 50 ms grow, the fix is a dirty-rectangle blur over the changed texels, which is parked behind that measurement and not built; the **map view's raster rebuild on resize** (`layout()` resamples `W × H` on every fitted-size change, i.e. every frame of a window drag) → the same pass, as a slow 1280 → 2560 px drag with the map open; **`.serena/`**, an MCP server's cache that appeared untracked in the working tree → not committed and not ignored, a `.gitignore` line is the human's call (as `docs/ROADMAP_OLD.md` stays untracked and left alone).
- **Deleted**: nothing.
- **Still owed by earlier sections**: unchanged — the reference-machine soak by hand, the M5 repair-by-E and the hand-lamp preview in a played hour, the generated-report freshness stamps in `calibration.md` / `EXPERIMENTS.md` / the snapshot, M6's two unrun checks, and the controls walkthrough itself. The waiver of T5–T8 did not clear any of them; it removed the occasion on which four of them were scheduled to happen.

## Re-read at the blockers task (2026-09-05, T1's three decision rows written and built — `ECONOMY_FIX_REPORT.md` §9)

- **Kept, with a phase**: **every front edge physical — a claim lays its segment's turrets from stock** (D-P4-9's other half, D-B1-4's count, GA-B6-4, the steel cost D-P4-4's) → **after Gate B**; the code it needs was deleted, not hidden, and `hour.ts`'s `carryTurrets`, `turretSpots`, `idleTurret` and `hqTurrets` come back from **commit 6694b71**, which is the last commit that has them. It cannot be built before the ring feed learns to keep feeding an edge that has a turret on it (`hookSyncEdges`), which is what makes a carried turret worse than none today. **West's coal made real** (D-P4-10 (a)'s other half — a physical Excavator and belt on west's rubble at the claim) → **after Gate B, and behind D-P4-12**: the tile layer has no coal rubble kind at all (`rubbleOf` gives stone / copper / steel / null, and west is a rail yard, so it reads `ind` and digs steel) and no coal quantity for the lot, both undecided constants, so this is a human's row before it is anyone's code. **The second Shot line / the Mk2 purchase from the idle steel** (D-P4-11) → Gate B, unchanged. **North after the hour** is no longer parked on a fall: at minute 65 it stands to 75:00 on every seed; what is still parked is what it runs on — the Generators are dry from 67:51–68:01 and D-P4-12 is what refills them.
- **Parked here from this task**: **north's minute is now a claim nobody has seen** — 65:00 is past the hour, so `E-hour` never reaches it and only `E-hour-north` (measured, not scored) does; if the hour is ever re-scoped to 75 minutes, north's claim becomes a scored check and wants its own DoD line. **`HOUR_END.held` is 3 and the HQ's white border never appears in a 3,600 s run** — §11's enclosure and the Electricians walking out of civic north are real beats that hour one no longer contains, and Phase 5's opening should say whether that is the shape of the hour or a hole in it.
- **Deleted**: **"north at 60–75 with real coal, unmeasured until D-P4-10"** (parked at the economy fix, 2026-09-04) — D-P4-10 is decided and the north-minute half is built and measured; what survives of it is the coal half, kept above behind D-P4-12. **"the minute-15 steel cluster"** (parked 2026-09-05) — D-HOUR-2 decided, built at minute 12, steel's minimum 30 at 12:01–12:02. **"the engineer knocked down in the 600 s edge rescue on seed 5 at 35:00"** (parked 2026-09-05, E-rifle 10 / 12) — the full suite is 13 experiments and **0 failing checks** with E-rifle back to 12 / 12; nothing is owed.
- **Still owed by earlier sections**: unchanged — the reference-machine soak by hand, the M5 repair-by-E and the hand-lamp preview in a played hour, the generated-report freshness stamps, the light-map blur's browser frame cost and the map view's resize raster, and the four STANDARDS rows plus the stranger test that the waived walkthrough was carrying. None of them moved.

## Re-read at the no-fall hour (2026-09-05, `PROGRESS.md` T2 done — `SLICE_REPORT.md` "M6 re-run after the economy fix")

- **Kept, with a phase**: unchanged from the blockers task's re-read above — **every front edge physical** (D-P4-9's other half, after Gate B, restore from commit `6694b71`) and **west's coal made real** (D-P4-10 (a)'s other half, after Gate B and behind **D-P4-12**). T2 measured; it parked no new work of its own.
- **Parked here from this task**: **the first shade is now a question a bot cannot answer.** §11 and E3 put it at minute 32–47; six runs print "never" and it has outlived the economy fix, the decided minutes and the no-fall hour. It is E-hour's only remaining finding. Either §11's minute is wrong for a two-claim hour, or the bot never lets a block sleep past d = 0.3 where a human would — and **only a played hour separates them**, which the T5–T8 waiver removed. → **a real Gate B**, or T9's re-scoping, whichever the human chooses.
- **Deleted**: nothing. The blockers task's re-read already retired the three items T2 would otherwise have closed (north's minute, the minute-15 steel cluster, the E-rifle seed-5 knockdown).
- **Still owed by earlier sections**: unchanged — the reference-machine soak by hand, the M5 repair-by-E and the hand-lamp preview in a played hour, the generated-report freshness stamps, the light-map blur's browser frame cost and the map view's resize raster, and the four STANDARDS rows plus the stranger test the waived walkthrough was carrying.

## Re-read at E-rifle at tile scale (2026-09-05, `PROGRESS.md` T4 done — `SLICE_REPORT.md` "E-rifle at tile scale", `EXPERIMENTS.md` `E-rifle-tile-steady` / `E-rifle-tile-rescue`)

- **Kept, with a phase**: unchanged from the no-fall hour's re-read — **every front edge physical** (D-P4-9's other half, after Gate B, restore from commit `6694b71`), **west's coal made real** (D-P4-10 (a)'s other half, after Gate B and behind **D-P4-12**), and **the first shade is a question a bot cannot answer** (→ a real Gate B, or T9's re-scoping). T4 sharpens the last of those rather than moving it: **no shade kills anything in 48 tile rescue runs** — every fall is `unfed` — and `E-hour` spawns **0 shades in six hours**, so what the played hour is being asked for is now "does a human let a block sleep past d = 0.3", not "when does the first shade arrive".
- **Parked here from this task**: **the 600 s belt is the only scenario in which the rifle changes an outcome**, and it is not §11's rescue (§11's belt is 90 s, at which 12 of 12 scenarios hold in both arms). The rifle's justification under §24 risk 10 rests entirely on a belt that never comes. → **the human's call in the T4 report** (decision 3); nothing is built on it either way. Also parked: **§5's rescue sentence still quotes the block sim** (a shade at an unlit edge in ~1.5 min) where the tile sim says crawlers, never a shade → **decision 1 of the T4 report**, a doc edit behind a human's word, not a constant.
- **Deleted**: **"E-rifle at tile scale"** (parked at Step 0, 2026-09-04, "the rifle never decides a rescue: twelve of twelve hold either way") — it does decide one at tile scale: 7 falls without, 4 with, 3 saved, 3 of the rest delayed, and the rifle never loses a block sooner. **The §19 reporting gap found during this task was closed in it, not parked** — `engineer.shootS` and `engineer.danger` always accumulated on the tile path, they were simply not in the tile table; two columns and two checks now score the 10 % and 5 % caps on §11's hour (0.50 / 0.69 / 0.06 % shooting, 0.00 % danger), and E-rifle is 14 checks.
- **Still owed by earlier sections**: unchanged — the reference-machine soak by hand, the M5 repair-by-E and the hand-lamp preview in a played hour, the generated-report freshness stamps in `calibration.md` / the snapshot, M6's two unrun checks, the light-map blur's browser frame cost and the map view's resize raster, and the four STANDARDS rows plus the stranger test the waived walkthrough was carrying. None of them moved.

## Re-read at Gate B (2026-09-05, passed — `docs/GATE_B.md`, verdict `proceed — with conditions`, closed as fully tested)

The gate constitution rule 9 names. **It was played and closed as fully tested**, which unblocks every item that was waiting for the gate to *happen*. The items that were waiting for a *measurement from a played hour* were not measured — §19's 10–30 and 30–60 windows were not recorded and no telemetry was read — so they are **not gate debts any more; they are Phase 5 observations**, each carrying its phase as rule 9 requires. Both groups are separated below rather than merged.

- **Unblocked by the verdict — they were parked "after Gate B" and the gate is now passed.** **Every front edge physical** (D-P4-9's other half, D-B1-4's count, GA-B6-4, D-P4-4's steel cost; restore `carryTurrets` / `turretSpots` / `idleTurret` / `hqTurrets` from commit `6694b71`, and the ring feed must first learn to keep feeding an edge that has a turret on it, `hookSyncEdges`) → **Phase 5**, after T9. **West's coal made real** (D-P4-10 (a)'s other half) → **Phase 5, still behind D-P4-12**, which is the undecided pair of constants (no coal rubble kind exists in `rubbleOf`, and the lot has no coal quantity) — the gate does not create them. **The second Shot line / the Mk2 purchase from the idle steel** (D-P4-11) → **Phase 5**, a human's row still. **The calibration's pip bands** (16–31 min, the lattice's) → the human's call now due: recalibrate at tile level or retire the bands with the lattice appendix.
- **Not settled by the gate, and no longer owed to it — they move to Phase 5 as observations.** These were all parked "→ Gate B" for a measurement the session did not take. **The first shade** (§11 / E3 say 32–47; six bot hours say never; 48 tile rescue runs kill nothing with a shade) — neither confirmed nor contradicted, so it stays **`E-hour`'s open finding** → **Phase 5**, closed by any later session that reaches minute 45, or by an experiment that makes a block sleep past d = 0.3 on purpose. **A player's walking share**, **the first pip**, **shades on lit tiles in play** and **a tile-level brownout soak** → **Phase 5**, with the telemetry panel open; until then the bot's numbers are the ones every report quotes, and they are tagged as the bot's. **Copper for repairs in the hour** and **the Playwright check of M5's repair by E and `?handlamp=1`** → **Phase 5**; neither was exercised. **Light on the map view** → **Phase 5**; the session played the world view. **The steel budget of hour one** and **the game's chest against the harness's** (D-P4-4 / D-B1-1) → **T9**, settled on bot evidence and a human's word rather than on play, which is what the waiver of T5–T7 bought. **The frame-time cost of the light map** (five bursts over 50 ms on the bot's walks) → still the reference-machine soak, by hand, unchanged by the gate.
- **Answered, and closed here.** **"The kerb reads lit, the road does not"** is *not* closed — the street question was never put — but the dark it belongs to is: **STANDARDS row A.7 is closed** on *"Yea the dark is pretty good so far"*, so **D-B5-1 stands play-supported** and the standards audit's seventh doc disagreement (D-B5-1 against the personal-light norm) is settled the doc's way; the hand-lamp *preview* item stays parked, since the preview UI was not exercised. **Gate B's new rifle row, "first fire minute and whether it mattered", is answered**: it was fired and it did not matter — *"I was just shooting at things"* — which is the play half of **D-R1** and agrees with T4's bots at §11's 90 s belt. **The engineer knocked down in the 600 s edge rescue** (parked at the economy-fix task for Gate B's rifle row) is closed by T4's re-measure: **0 knock-downs in 24 rifle runs**, HP min 34.
- **Parked here from the gate.** **D-GB-1's hybrid loop** — the claim still burns off its block, and powering and lighting the next area becomes a physical expedition the engineer walks, with lighting-up as a threat trigger → **Phase 5 / 6**, the milestone that builds it, and the doc sentences it moves (§5's claim loop, §10's triggers, §11's hour, §19's asks) move there with a changelog line, not before. **D-GB-2, knowing what to do next** — the hour's one named problem, and constitution rule 8 forbids tutorial screens, so the rule-8-legal forms (a lit objective on the map, a pip on the thing that wants feeding, a marker on the next area's box) go first and a quest system is the human's to take → **Phase 5, gate condition**. **D-GB-3, rifle range** → **Phase 5, gate condition**; it is a constant the doc does not carry. **D-GB-4, enemy density on the walk and at the base** → **Phase 5, gate condition**, and it needs no rule change: §19 allows 10 % shooting and 5 % danger where the measured hour is 0.50 / 0.69 / 0.06 % and 0.00 %. **The burn-off question was asked at the gate and displaced by the loop redesign rather than answered** → **Phase 5**, and D-GB-1's expedition is what will reshape it anyway.
- **Deleted**: nothing. **Still owed by earlier sections**: the reference-machine soak by hand, the generated-report freshness stamps in `calibration.md` / the snapshot, M6's two unrun checks, the map view's resize raster, and — because T5, T6 and T7 stay waived — the controls walkthrough's four STANDARDS rows (4.3, B.3, B.6, C.2), the two-assembler line and the stranger test. None of them moved. **Gate B itself owes nothing**: T8 is `done`.

## Re-read at Absorb Gate B (2026-09-05, `PROGRESS.md` T9 done — `PROGRAMME_STATE.md` B.31, `SLICE_REPORT.md` "Absorb Gate B")

The last Phase 4 re-read. T9 wrote doc, not code, so it parks little and clears one item that was addressed to it by name.

- **Kept, with a phase**: unchanged from the Gate B re-read above. **Every front edge physical** (D-P4-9's other half; restore `carryTurrets` / `turretSpots` / `idleTurret` / `hqTurrets` from commit `6694b71`, and `hookSyncEdges` must first keep feeding an edge that has a turret on it) → **Phase 5**, and it is now the first unblocked item on that list. **West's coal made real** (D-P4-10 (a)'s other half) → **Phase 5, behind D-P4-12**. **The second Shot line / the Mk2 purchase** (D-P4-11) → **Phase 5**. **The calibration's pip bands** (16–31 min, the lattice's, against ≈ 3 / ≈ 6 min at tile level) → **the human's call, now due**: recalibrate at tile level or retire the bands with the lattice appendix.
- **Deleted**: **"the steel budget of hour one, and the game's chest against the harness's (D-P4-4 / D-B1-1) → T9"** (parked at the Gate B re-read). T9 is this task and there is nothing left in the item: both rows are `decided` (the human's message of 2026-09-04) and built, `docsync:check` is green on them, and the play evidence that could have reopened them was never taken — the two-assembler line (T6) stays waived and the played hour read no telemetry. It is closed as settled on bot evidence and a human's word, which is what the waiver bought, and it does not travel to Phase 5 as a debt.
- **Parked here from this task**: **the two `[play: Gate B]` tags that were placed in the doc before the gate was played** (commits `27cc9b0`, `906044c`) are a process finding, not a code one: a tag naming a gate that has not happened cannot be checked by `docsync`, and nothing in the repo would have caught either of them → **Phase 5's opening**, as one line in whatever check the phase adds, or a deliberate "we accept it and re-read tags at every gate". **D-GB-1's hybrid is now recorded in two places** — `DECISIONS.md` and §10 — and both say the same thing about which sentences move when it is built; if the build ever lands without moving §5, §11 and §18, those two records are what will catch it → **the milestone that builds it**.
- **Still owed by earlier sections**: unchanged and untouched by T9 — the reference-machine soak by hand, the generated-report freshness stamps in `calibration.md` / the snapshot, M6's two unrun checks, the light-map blur's browser frame cost and the map view's resize raster, and the controls walkthrough's four STANDARDS rows (4.3, B.3, B.6, C.2) plus the two-assembler line and the stranger test that T5–T7's waiver left unrun. The Phase 5 observations the gate handed over — the first shade, a player's walking share, the first pip, shades on lit tiles, the tile-level brownout soak, the burn-off question, copper for repairs, the repair-by-E Playwright check and light on the map view — all stand exactly as the Gate B re-read left them; T9 measured nothing and moved none of them.
## Re-read at the Phase 5 opening (2026-09-05, `PROGRESS.md` T10 done — `PHASE_5_REPORT.md`)

The first Phase 5 re-read. The opening built nothing, so it parks nothing new from code; what it does is give the phase's inherited list its milestones and close the one item that was addressed to this opening by name.

- **Given a milestone, from the Phase 5 list**: **every front edge physical** (D-P4-9's other half, and `hookSyncEdges` learning to keep feeding an edge that has a turret on it) → **M2**, with the machine list and the command layer, because it is placement of a §13 machine on a real tile and nothing else. **West's coal made real** (D-P4-10 (a)'s other half, behind D-P4-12) → **M1**, with the rubble model, which is the task that adds a rubble kind at all. **The second Shot line / the Mk2 purchase** (D-P4-11) → **M1**, where the recipes become makeable; it needs a human's row first. **Rubble sinks for stone** (concrete, parked at Phase 0) → **M1** for the recipe, and the Mixer that makes it is **D-P5-2**'s question, because its unlock is found tech.
- **Deleted**: **"the two `[play: Gate B]` tags placed before the gate was played → Phase 5's opening, as one line in whatever check the phase adds"** (parked by T9). This is that opening and the item has an answer: `docsync:check` gains one rule in **M1** — every `[play: <gate>]` tag must name a gate whose evidence file exists and carries a verdict — which is a dozen lines in `packages/tools/src/docsync.ts` and closes the one class of provenance error the repo cannot see. The alternative T9 offered ("accept it and re-read tags by hand at every gate") is not taken, because it is the option that already failed once. It leaves this list as a build item on M1, not as a deferred one.
- **Kept, unmoved, and now the phase's own**: **the calibration's pip bands** (16–31 min, the lattice's, against ≈ 3 / ≈ 6 min at tile level) → still recalibrate-or-retire, a human's word, and the Phase 5 **gate** is where the lattice appendix goes either way. **The deletions due at the Phase 5 gate** — `--map lattice` / `?map=lattice`, the lattice fixtures and results, the `?flow=0` free camera and the block-only harness start ring (`sim.ts syncEdges`, GA-B1-15) — are unchanged and dated to the gate, not to a milestone.
- **New, parked by the opening**: **§11's "30–60 min" paragraph describes minute 65** — north's claim, the HQ's white border and the Electricians all sit past the hour by D-P4-10, so §11's own heading spans more than its window → **the human**, either as its own heading or as the 75-minute re-scope that `E-hour-north` already measures green (`PHASE_5_REPORT.md` §6). **D-GB-1's hybrid has no milestone**: it is decided and not built, it changes the loop rather than completing the factory, and putting it inside a phase that is already six weeks of §12–§14 makes one milestone do both jobs → **the human names its phase** (the opening recommends Phase 6).
- **Still owed by earlier sections**: unchanged. The reference-machine soak by hand, the generated-report freshness stamps, M6's two unrun checks, the light-map blur's browser frame cost and the map view's resize raster, the controls walkthrough's four STANDARDS rows (4.3, B.3, B.6, C.2), and the two-assembler line — which is no longer only a waived T6 but **Phase 5's own DoD**, and is `PROGRESS.md` **T18**. The Phase 5 observations the gate handed over — the first shade, a player's walking share, the first pip, shades on lit tiles, the tile-level brownout soak, the burn-off question, copper for repairs, the repair-by-E Playwright check and light on the map view — stand exactly as the last two re-reads left them; the opening measured nothing.

## Re-read at T11, the Phase 5 decisions (2026-09-05, `PROGRESS.md` T11 done — `PHASE_5_REPORT.md` §12)

T11 wrote in the human's answer to fourteen rows and wrote no code, so it parks nothing new. It closes two items the opening had just parked, and it puts a date on a third.

- **Closed: "§11's '30–60 min' paragraph describes minute 65."** Parked by the opening as either-heading-or-re-scope. **D-HOUR-3 re-scoped the hour to 75 minutes**, so north's claim becomes a scored check with its own DoD line and the HQ's white border and the Electricians stop being beats a 3,600 s run can never reach. It is no longer deferred: it is **M1** work, because §11's minute table is generated by docsync from `constants.ts` `HOUR`, so the re-scope is a code change first and a doc change by regeneration. The DoD line lands with it.
- **Closed: "D-GB-1's hybrid has no milestone."** Parked by the opening because the row is decided and not built. **D-GB-1-rider gives it Phase 6**, with its own milestone and its own doc-edit pass — it moves §5, §10, §11, §18 and §19, so it never becomes a corner of a Phase 5 milestone.
- **Newly dated, not newly parked: D-P4-3's placeholder deposits.** The row was decided as recommended — the 160-tile placeholder stays inert until Phase 9's generator validator — but `E-coal` and `E-tram` may need the outskirts deposits live to measure depletion and haul. **M1's report must say which half it built.** If a Phase 5 run needs them real, the row is reopened rather than quietly widened; that is the tripwire, and it is written into the row itself.
- **Kept, unmoved**: the calibration's pip bands (still recalibrate-or-retire, a human's word), the reference-machine soak by hand, the generated-report freshness stamps, M6's two unrun checks, the light-map blur's browser frame cost, and the map view's own §26 line. **D-B2-2, C3 and D-SA-2** are not deferred — they are `provisional` under rule 14 as Claude's call and are named in T13's and T17's blocked-by lines, so they surface at the milestone that needs them rather than in this file.

...
## Re-read at Gate B (2026-09-05, played in part — `docs/GATE_B.md`, verdict `proceed — with conditions`)

The gate constitution rule 9 names. **It was played once, minutes 0–10, with no telemetry panel read**, so it unblocks the items that were only waiting for the gate to *happen* and answers almost none of the items that were waiting for a *measurement from a played hour*. Both groups are separated below rather than merged.
...
- **Still owed, and the gate did not settle them, because the session stopped at minute 10 and read no telemetry.** **The first shade** (§11 / E3 say 32–47; six bot hours say never; 48 tile rescue runs kill nothing with a shade) — the session never reached minute 30, so it neither confirms nor contradicts → **one played session that reaches minute 45**, which also fills §19's empty **10–30 and 30–60** rows. **A player's walking share**, **the first pip**, **shades on lit tiles in play** and **a tile-level brownout soak** → the same session, with the telemetry panel open. **Copper for repairs in the hour** and **the Playwright check of M5's repair by E and `?handlamp=1`** → the same session; neither was exercised. **Light on the map view** → unanswered; the session played the world view. **The steel budget of hour one** and **the game's chest against the harness's** (D-P4-4 / D-B1-1) → unanswered; the harness's numbers stay the ones the reports quote. **The frame-time cost of the light map** (five bursts over 50 ms on the bot's walks) → still the reference-machine soak, by hand.

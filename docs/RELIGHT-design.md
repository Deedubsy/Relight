...
| Targets | nearest lamp, then turret, then the substation (40 unshot arrivals stop it) [sim: B-M4-threat]; the engineer only when shot or blocked by them (D5) | substation (disables 30 s each, stacking) | barricade → turret → substation |
...
**Retaliation (D5).** The rot does not know the engineer is there. A crawler keeps its chain (nearest lamp, then turret, then the substation) and turns on the player only when shot by them or when the player stands in its path; a shade and a hulk never retarget at all. A crawler that has turned on the player hits for 5 HP a second at arm's reach (GAME-ASSUMPTION; §25 item 18) [sim: B-M4-threat]; the engineer has 100 HP, regains it at 5 HP/s after 5 s out of contact, and at zero is knocked down and respawns at the HQ with their inventory intact, no other penalty. Rot on the pavement and Dark blocks do nothing to the player: the engineer walks anywhere. The rifle fires Shot magazines from the engineer's pockets at turret rules: 1.5 rounds a second, so a crawler (12 HP, 3 rounds) takes ≈ 2 s against a turret's 0.6 s, shades are untargetable on unlit tiles, hulks take 1 damage a round and are only slowed, and the rifle's range is a turret's 9 tiles, no more. One upgrade at the Arsenal (a second barrel: 1.4 s a crawler). The game is completable without firing a round; the rifle exists for the rescue in §5, and a bot that never fires loses the same blocks as one that does **[sim: E-rifle]**.
...
| Shot magazine | 2 steel + 1 Cu | 1 magazine (10 rounds) | 6 s | Assembler (Mk1) |
| Shot magazine (Mk2) | 2 steel + 1 Cu | 1 magazine (10 rounds) | 3 s | Assembler Mk2 (the purchase; unlock open, D-B1-1) |
...
| Assembler | 3×3 | 100 kW | any intermediate recipe (Shot magazine: 2 steel + 1 Cu in 6 s, 10 mag/min — the Mk1; the Mk2 makes it in 3 s, 20 mag/min, D-P4-4 / D-B1-1 **[sim: M2-rates, B-M6-hour]**) | start (Mk1) · purchase (Mk2, unlock open) |
...
| Floodlight | 2×2 | 40 kW | 12-tile cone, 60° wide, rotatable (R); lit while its face is powered, at any brownout throttle above zero; dark only on a dead grid, like the Lamps (D-B3-4) **[sim: B-M3-unlocks, M3-rates]** | Electricians |
...
**Pre-existing fixtures (D-B1-4).** A block's substation (3×3) sits on the buildable tile nearest its lot's centroid; its streetlights stand on the kerb — the street tiles bordering the lot on its side of the watershed — one every 4 tiles along each street segment, 3 in 8 broken, each lighting a 7-tile radius — to the street's midline, so a held face lights its half of the shared street (D-B5-4: the kerb row 65–93 % lit and the half-street 44–65 % at radius 7, against 50–68 % and 26–41 % at the old radius 4; the Lamp keeps its 4 **[sim: B-M6-light]**); the HQ's start turrets are one per 16 tiles of each segment's length, at least one, placed on the segment's front ring facing its street. All three are placed by the face's geometry, not by the lattice's lot coordinates.
...
|---|---|---|---|
| Assembler rate (Mk1) | 10 mag/min | §12 ammo chain, §13 Assembler row | D-P4-4, D-B1-1 (C9, D-P3-7 superseded) |
| Assembler rate (Mk2) | 20 mag/min | §12 ammo chain, §13 Assembler row | D-P4-4, D-B1-1 (C9, D-P3-7 superseded) |
| Shot magazine | 2 steel + 1 Cu → 10 rounds in 6 s (Mk1) · 3 s (Mk2) | §12 recipe table | D-P4-4 |
...
| Start turrets | 6, hoppers full, spread over the HQ's segments by length | §11 0–10 min, §13 pre-existing fixtures | D-P4-8 |
| Streetlight radius | 7 tiles, to the street midline (the Lamp keeps 4) | §13 pre-existing fixtures | D-B5-4 |
...
**Flow visibility (can the player see the bottleneck?).** Yes, in two places. In world view, empty belt segments are dashed grey and a jam has a static red marker at the stop tile. In map view, each frontage edge carries a supply pip, so "which edge is starving" is a colour on the map, not a tour of the line. Power shortfall is one HUD number, the percentage every machine is running at; nothing stops, the whole line slows, and the pips say where it bites first **[sim: E2-matrix]**. The thing the player *cannot* see directly is the rot growth rate of a block, only its density; that is intentional, but it is the first thing to expose if testers say fights feel random.
...
- **Enemy AI beyond flow-field.** Each bloom follows a flow field to its target class. No formations, no retreat, no learning. [sim: B-M4-threat]
...
- **Turrets that need power.** Would let a brownout collapse the front instantly. Rejected; turrets need only ammo, so ammo and power are two separate failures with two separate fixes. Proportional brownout (section 14, D-B3-4) keeps the ammo dependency one step removed: a brownout slows the Shot assemblers, the buffer and then the hoppers drain, and the unfed rule takes the block. The difference relied on is speed: a 50 % shortfall takes over an hour to show its first red pip and the first fall is 3–8 minutes behind it, not instant **[sim: E2-sustained6h-h500-50pct]**.
...
7. *An early brownout sheds the HQ first.* Retired by D-B3-4: power never stops a substation. An early brownout now slows the §11 line — one Generator against the whole line placed at 0:00 runs the hour at 42–73 % and still holds the HQ with 500 magazines made **[sim: B-M4-gate]** — and the 40 coal in hand still runs out at minute 11 without the patch **[sim: E4h-literal-1gen]**. Mitigation unchanged: the §11 start conditions (coal patch, a Generator at minute 6, a third at the east claim and a fourth at minute 45), under which hour one never browns out, at block level and on the tiles **[sim: E4h-gen@0,6,15,25, B-M4-gate]**.
...
9. *Cascade during a shortfall.* Retired by D-B3-4: with no shedding there is nothing to cascade; a 10-minute shortfall of up to 50 %, or one held to the fifth hour, costs 0 blocks at 500 kW or 1 MW of headroom **[sim: E2-matrix]**. What remains is the slow starvation of the ring under a shortfall that is never fixed (section 14), which the red pip leads by 2–8 minutes **[sim: E2-sustained6h-h500-50pct]**.
...
2. *Pathfinding for 40-crawler blooms across many edges.* One flow field per bloom per target class, cached per block edge; enemies never path across more than two blocks. [sim: B-M4-threat]
...
14. **Power in hour one.** Closed on the draw. Front and Contested substations draw 100 kW and interior 20 kW (§5; 200/40 had no evidence behind it), and ammo, not power, is the hour-one lesson. At that draw three Generators carry hour one to minute 45 and a fourth is needed for the fifth Excavator and third Assembler (980 kW peak, two-thirds of it machines) **[sim: E4h-gen@0,6,15, E4h-gen@0,6,15,25]**; §11 says four, not five. Settled by D1: the HQ coal patch is ~700 units and mines out at about minute 29 at one Excavator, so west is wanted by minute 30 **[sim: E4h-patch700]**. Locked at Gate A (D-P3-1): the draw stays 100/20 kW and the hour-one lesson is ammo (§11). Confirmed on the tiles before M4 (D-P4-7 settled): with the E4 schedule — Generators at 0, 6, 15 and 45 minutes, the line's machines at E4's minutes — the §11 hour holds on seeds 3 / 4 / 5 with 0 brownout seconds, a peak of 1,090 kW on 1,200, 806 / 801 / 806 magazines, and the HQ patch's coal gone from the Depot at 60:00 (639 / 621 / 621 burned), so west is wanted by the hour, as E4 said **[sim: B-M4-gate]**; the M3 measurement's 350 vs 300 kW was the pre-E4 hour, one Generator against the whole line at 0:00.
...
- §5, §14, §19, §23, §24 risks 7 and 9, §13 Floodlight — proportional brownout replaces the shed order: when demand exceeds supply every machine on the grid runs at supply ÷ demand, nothing switches off, nothing has an order, no substation is ever stopped by power (substations stay binary and stop only under the unfed rule), lights are lit at any throttle above zero and dark only on a dead grid, belts and turrets are never slowed; a dead grid is back the second a Generator burns — D-B3-4, the human's decision 2026-09-04: the shed order was self-defeating at tile level (the M3 soak's dead grid) and unreadable in play; the sim, the tests and the game follow it — E2-matrix, E2-sustained6h-h500-50pct/75pct/90pct, M3-rates
- §5, §14, §23, §24 — what a sustained shortfall costs under D-B3-4, measured: nothing for 10 minutes at up to 50 % or for an hour at 5–50 %; from hour 4 to hour 10 the ring starves behind the red pip — first pip 67–216 min in at 50 %, 18–35 at 75 %, 12–16 at 90 %, the first fall 1.7–7.5 min after it, the pip leading in every run — E2-sustained6h-h500-50pct, E2-sustained6h-h500-75pct, E2-sustained6h-h500-90pct
- §11, §24 risk 7, §25 item 14 — the tile-level §11 hour re-run on the E4 schedule (Generators 0 / 6 / 15 / 45 and 0 / 6 / 15 / 25, machines at E4's minutes, the coal Excavator on the HQ patch belted into the Depot): 0 brownout seconds, the HQ holds on seeds 3 / 4 / 5, 806 / 801 / 806 magazines, the patch's coal out of the Depot at 60:00; the M3 measurement (350 vs 300 kW, the assembler shed at 0:20, the HQ lost at 12:25) was the stale pre-E4 script, one Generator against the whole line at 0:00, which under D-B3-4 also holds the hour at 42–73 % with 500 magazines — D-P4-7 settled 2026-09-04 — B-M4-gate
- §7, §17 — the rot at tile level: crawlers and shades are bodies born on the segment ridge, walk a per-face flow field (lit lamps → past the turret → the substation, the outskirts pole where there is none) inside their two blocks, count the 40 unshot arrivals on the tiles, and turn on the engineer only when shot by them or stood in their path (D5); turrets shoot them out of their own hoppers at 4 HP a round; the turret is a waypoint the crawlers do not harm (D-B4-3) — prompt B M4, run name B-M4-threat
- §11, §19 — the §11 hour with real bodies on the E4 schedule with power on: 467 / 370 / 241 crawlers on seeds 3 / 4 / 5, all but 6 / 0 / 0 shot by turrets, 5–13 lights eaten and left dark, no shades in the hour, the HQ holds; the harness bot walks 2.6–4.1 % of the hour against §19's 15 % (a player's estimate, kept) — prompt B M4, run name B-M4-hour
...
- §11 — 0–10 rewritten around the ~6-minute red pip as the hand-feed beat: six full turrets, 20 magazines and 40 coal in the chest, Generator 2 at 6:01, the Mk1 at 10 mag/min, the second steel Excavator at minute 12 (steel's minimum 30 at 12:01) — D-P4-4 / D-P4-7 (b) / D-P4-8 decided by the human 2026-09-04; E-hour re-run 6 / 6 with no fall and no brownout on seeds 3 / 4 / 5 — B-M6-hour
- §11 — 10–30 and 30–60 rewritten to what the economy affords: two claims in the hour (east 15, west 25), Generator 4 at 45, the second copper Excavator at 46, the third Assembler at 50, north at 60–75; end counts 4 Generators / 7 Excavators / 3 Assemblers / 6 turrets / 3 Held; the coal patch dug out at ~36 — the old script fell on every seed, and north claimed at 65 falls on coal (Generators dry 67:51–68:01), not ammo — B-M6-hour, B-M6-hour-north
- §12, §13 — the Mk1 / Mk2 ladder returns: Shot magazine 6 s / 10 mag/min on the start Assembler, 3 s / 20 mag/min on the Mk2 (the purchase; unlock open); the §12 table follows `recipes.ts` (docsync) — D-P4-4 / D-B1-1 decided by the human 2026-09-04 — B-M6-hour
...
- §11 0–10 — the coal Excavator and its belt before Generator 2 at minute 6; the Shot line named at minute 8 — D-P4-7, D-HOUR-1
- §11 0–10 — the first empty hopper is fed by hand with E; the sentence about walking the chest's magazines to the turrets removed — D-P4-8
- §11 10–30 — the second steel Excavator at minute 15 (was 12 in the 0–10 window) — D-P4-7

...
**Grid.** 32 px tiles, orthographic, no elevation. The city is a grid of **block cells**, each 32×32 tiles: a 24×24-tile buildable lot with a 4-tile street margin on every side, so two adjacent cells share an 8-tile-wide street. A standard city is 24×22 cells (768×704 tiles): the sim's canonical map, which the §18 drawings use, is 24 wide and 22 tall with the river along the bottom row **[sim: E8-cadence]**; whether the shipped map is 24×24 is a human call (PHASE_1_REPORT.md, decision 1). Roughly 8–10 % of cells are **inert**: river, embankment, collapsed overpass. Inert cells cannot be built on and cannot hold rot.
...
3. Contested does three things at once: the cell's pre-existing streetlights come on (some are broken — those gaps are yours to fill with lamps); the rot begins burning off from the lamps outward; and the cell fires a **wake bloom** of twice its normal size, immediately (a wake bloom costs 63 rounds against 34 for a steady bloom on civic, 84 vs 49 on residential, 110 vs 82 on industrial, the 40-crawler cap biting on the denser blocks) **[sim: E3-compact-shade0.3-hulk0.5]**.
...
| Ammo, outskirts steady state | 4.3 mag/min + 2 shells/min **[sim: E3-block]** (4.2 + 1.9 per edge over 25 h of compact play **[sim: E9-district]**) | 0.13 mag/min |
...
**Rot wells.** 3–6 cells per map (always visible on the map as a darker throb) have `dmax + 0.3` and 4× growth, and lift the cap of cells within 3 blocks. A well dies when all four of its neighbours are Held and it has bloomed for five minutes with no dark neighbour to feed it. Killing a well is the mid-game milestone (section 15). The enclosure rule is in the sim and never fires under any bot: the compact bot claims the well block itself at hour 13–24 at the §18 cadence, before it is ever enclosed, so wells are taken head-on rather than starved **[sim: E8-wells]**.
...
| Punishes | ammo throughput and hopper distribution: an edge left unfed loses its block in about 7 min (4–14 across seeds) **[sim: E1-starve-substation-N40]** | light coverage: a single unlit gap in a street is a corridor to the substation | frontage length: one long thin front has nowhere with enough Cannons |
...
**What threat scaling feels like.** Civic blocks are the soft way in; residential is the default; industrial is where the steel is and where shades become constant (deep industrial blocks sit at d ≥ 0.3 and shade on every bloom; the first shades come from residential blocks that have slept past d = 0.3; hulks belong to wells and outskirts, 28–39 % of their blooms over 25 hours (9 % in the first five, while the wells are shallow) against 11 % of deep industrial **[sim: E3-compact-shade0.3-hulk0.5, E3-spike-shade0.3-hulk0.5, E9-district]**); outskirts are where the deposits are and they cost 3–4× a residential edge to hold **[sim: E3-block]**; wells are the boss blocks. The sim's spike policy (straight line toward a target) paid 2.7× the ammo of the compact policy at five hours on the scattered map, for 40 blocks against compact's 52 (16,792 vs 6,178 magazines; 1.3× on open ground with no inert cells, 17,475 vs 13,542) **[sim: E6-shape, E7]**. That ratio is the number the whole game is tuned around: shape is worth nearly three times your ammo budget where inert cells give compact play free walls (30 % of it on open ground).
...
1. **Shape vs reach.** The Foundry is 7 blocks north-east. A straight corridor of 7 blocks has 16 frontage edges; a 3×3 blob plus a 4-block spur has 14 and gets you a 3-block interior. The sim says the corridor costs 2.7× the ammo on the scattered map, where inert cells wall the blob (~1.3× on open ground) **[sim: E6-shape, at the §18 cadence]**, but the blob is 40 minutes slower to the Foundry. *Cost paid:* time vs ammo.
...
| Ammo demand (compact play) | 8–44 mag/min **[sim: E5-compact-base]** | 50–85 mag/min + 5–20 shells/min at four assemblers, which the front outgrows at hour 10–12 **[sim: E9-hourly]** | 85–180 mag/min + ~20 shells/min (four assemblers plateau at 85 because blocks fall; eight from hour 4 meet 165–180) **[sim: E9-hourly, E9-hold]** |
| Power | 1.0–2.0 MW **[sim: E4h-gen@0,6,15,25, E2-demand]** | 2.0–5.6 MW **[sim: E2-demand]** | 5.6–10.0 MW **[sim: E2-demand]**, then 40 MW for the Relight |
...
- **Strategy space.** Compact vs spike vs skirting: the sim shows a 2.7× ammo spread between compact and spike on the scattered map (1.3× on open ground); "always take the quietest block" costs 1.17× compact on the scattered map, because it never uses the free walls (0.57× on open ground) **[sim: E6-shape, E7]**. Steel-first (Foundry) vs Cannons-first (Arsenal). Coal-for-power vs coal-for-shells. Kill wells early vs wall them off.
...
**5 hours (map view, south-centre 14×9 blocks; the drawing shows 52 blocks held, front 24, interior 36, with 5 cells drawn as front that are interior by the 4-neighbour rule; the sim at 5 h at the §18 cadence holds 52, front 18, interior 36 [sim: E8-cadence]).** Compact policy [sim: E7, §18 recount]. Ammo demand ≈ 52 mag/min from 3–4 assemblers **[sim: E9-hourly]**; 14 shells/min from the Arsenal for the two outskirt edges near the well.
...
**25 hours (map view, full 24×22 city; the drawing shows 313 blocks held, front 67, interior 270, with 7 cells drawn as front that are interior; the sim at 25 h at the §18 cadence holds 292, front 39, interior 257 [sim: E8-cadence]; one or two of five wells taken by the compact bot [sim: E8-wells]; four of five wells dead, Power station under restoration).**
...
2. *The river T.* Claim east and west first so that the third claim (north) makes the HQ interior at minute 40 rather than minute 65–86 (the pure compact policy's first interior time) **[sim: E7]**; the player who spots this feels like they beat the rule.
...
2. *Ammo tuning: the front either never bites or starves everyone.* Mitigation: the tuning surface is four numbers per district (dmax, g, bloom base, bloom slope) and the sim already exposes them; blooms are capped and bursty, but a line ~2 % short at 3 h with an empty buffer lost 12 blocks in five hours on the spike policy, all on industrial and well edges, while compact play at 55 % load lost none **[sim: E1-ring-spike, E1-ring-substation-N40]**: the pip reddens slowly on a residential edge and a well edge simply falls.
...
8. *A straight push outruns its ammo line.* On the spike policy demand reaches 98 % of production at 3 h, the buffer is empty after 2 h, and 12 blocks fall in 5 h even with the ring fed in creation order (a well bloom is up to 170 rounds against a 100-round edge hopper). Mitigation: none in the rules; it is the price of shape, and §12 now states the demand **[sim: E1-ring-spike]**.
9. *Cascade during a shortfall.* Under substation-first shedding a 25 % shortfall for 10 minutes at hour 4 cost 5 blocks (3–6), the +80 kW each fallen block hands its neighbours making about one knock-on per two direct sheds. Mitigation: machines-first shedding (section 14), which cut it to 0, and 1 MW of headroom, which also cut it to 0 **[sim: E2-matrix]**.
...
5. **Endgame hold.** Re-run at 25 h on compact play at the §18 cadence, three seeds, with the wells' ×3 and the wake-size bloom on every awake block both in the sim **[sim: E9-hold]**. At four assemblers the window demands 138 mag/min against 85 the hour before (1.6×), peak minutes 2.25×; six assemblers 189 against 125; eight 236 against 165. Blocks lost in the twenty minutes from 25:00 when production only matches demand and nothing is banked: 20–28, 24–31 and 30–36, against 4 without the surge. A 4,000-magazine bank drains before the window opens and the hold still loses 34–38. Eight assemblers from hour 4 with an uncapped bank hold ~15,000 magazines (8,500–20,000) at 25 h and lose 0 blocks on all three seeds. D4 (bankable Relight) stands as the design; open: the bank a player can build and store by hand, a Phase 10 world-view question, and the assembler count the §12 late column assumes.
...
7. **The quiet-block problem** (risk 6 above). The first real-game playtest question is whether resource pull is enough to make players take dense blocks. On the scattered map the sim already prices quiet-block play above compact play (1.17×) **[sim: E6-shape]**; on open ground it is still 0.57×.
8. **Outskirts and well edge cost in play.** Closed at 25 h: over 25 hours of compact play an outskirts edge costs 4.2 mag/min + 1.9 shells/min (hulks in 39 % of blooms) and a well edge 5.2 + 1.4 (28 %), against 4.3 + 2.0 and 5.3–8.7 + 2.1–2.6 at isolated steady state **[sim: E9-district, E3-block]**.
...
11. **Late-game demand.** Closed at 25 h. Compact play at the §18 cadence demands 52 mag/min at 5 h, 73 at 8 h and 84 at 10 h, where four assemblers plateau and blocks start falling (144–164 lost by 25 h); eight assemblers from hour 4 meet 165–180 at 25 h with ~20 shells/min **[sim: E9-hourly, E9-hold]**. §12's mid and late columns carry these numbers.
...
13. **Spike on the scattered map.** Reopened (D-25-13). At the §18 cadence spike costs 2.7× compact on the scattered map and, with the ring fed in creation order, loses 12 blocks (12–13) in five hours while holding 40 against compact's 52 **[sim: E7, E1-ring-spike]**. Whether that is a choice or a trap is a human verdict (PHASE_1_REPORT.md, decision 2).

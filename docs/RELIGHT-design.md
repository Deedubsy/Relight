...
| Gun turrets (range 9) | 2 (1–3 by segment length) | 1 per 16 tiles |
...
**Outskirt blocks.** No substation and no streetlights. To hold one you place a craftable **Substation** (Electricians survivor, section 8) and your own lamps. Same rules otherwise; the district cap is 1.0, so these are the hardest edges in the game.
...
| Workbench | 2×2 (on the HQ lot) | — | hand-crafting within reach, from the engineer's pockets, at the recipe's own time | start |
...
- **Player combat as a system.** The engineer walks and carries (D5), but there is no player-side combat layer: the rifle uses the turret's damage rules with a slower fire rate, there is no aiming skill, no hit chance, no cover, no ammo types beyond Shot, one upgrade. Being knocked down costs a walk from the HQ and nothing else: no hunger, no death penalty, no dropped inventory, no armour, no healing items.
